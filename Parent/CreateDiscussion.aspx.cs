    ...
    public partial class CreateDiscussion : Page
    {
        ...
        private void LoadTags()
        {
            try
            {
                ...
                using (var cmd = new SqlCommand("SELECT TOP 5 tagName FROM dbo.Tag ORDER BY NEWID()", c))
                {
                    c.Open();
                    using (var r = cmd.ExecuteReader())
                    {
                        ...
                        while (r.Read())
                            sb.Append("<span class='pt-tag-pill'>" + System.Web.HttpUtility.HtmlEncode(r["tagName"].ToString()) + "</span> ");
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    if (!string.IsNullOrEmpty(rawTags))
                    {
                        ...
                        foreach (string rawTag in tagNames)
                        {
                            ...
                            using (var cmd2 = new SqlCommand("INSERT INTO dbo.ForumTag(forumTagId,forumId,tagId) VALUES(@id,@fid,@tid)", c, txn))
                            {
                                ...
                                cmd2.Parameters.AddWithValue("@fid", forumId);
                    ...
                    using (var cmd = new SqlCommand("SELECT parentId FROM dbo.[Parent] WHERE userId=@u", c, txn))
                    {
                        ...
                        if (r != null && r != DBNull.Value) parentId = r.ToString();
                    }
                        ...
                        using (var cmd = new SqlCommand("SELECT s.userId FROM dbo.StudentParent sp INNER JOIN dbo.Student s ON sp.studentId=s.studentId WHERE sp.parentId=@p", c, txn))
                        {
                            ...
                            using (var rdr = cmd.ExecuteReader())
                            {
                                ...
                                while (rdr.Read())
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(uid)) childUserIds.Add(uid);
                                }
                                rdr.Close();
                            ...
                            txn.Commit();
                            Response.Redirect("ForumThread.aspx?forumId=" + forumId, false);
                            Context.ApplicationInstance.CompleteRequest();
                        }
                        catch
                        {
            ...
            {
                ShowMsg(T("Error creating discussion.", "Ralat membuat perbincangan."), false);
            }
        }
