...
namespace ScienceBuddy.Parent
{
    public partial class EditStudyPlan : Page
    {
        ...
        private string _planId
        {
            get
            {
                return ViewState["PlanId"] as string ?? "";
            }
            set
            {
        ...
        private void LoadChildren()
        {
            ...
            try
            {
                string sql = @"SELECT s.studentId, ISNULL(s.nickname, s.name) AS displayName
                ...
                using (var cmd = new SqlCommand(sql, conn))
                {
                    ...
                    {
                        while (reader.Read())
                            ddlSidebarChild.Items.Add(new ListItem(reader["displayName"].ToString(), reader["studentId"].ToString()));
                    }
                }
            }
            catch { }
            ...
            if (ddlSidebarChild.Items.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(savedChildId) && ddlSidebarChild.Items.FindByValue(savedChildId) != null)
        ...
        private void LoadSPId()
        {
            ...
            {
                using (var conn = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("SELECT studentParentId FROM dbo.StudentParent " +
                    "WHERE parentId = @parentId AND studentId = @studentId", conn))
                {
                    cmd.Parameters.AddWithValue("@parentId", _parentId);
                    cmd.Parameters.AddWithValue("@studentId", _selectedChildId);
                    conn.Open();
                    object result = cmd.ExecuteScalar();
                    if (result != null && result != DBNull.Value)
                        _studentParentId = result.ToString();
                }
            }
            catch { }
        }
        ...
        private void LoadPage()
        {
            ...
            if (string.IsNullOrEmpty(planId))
            {
                ...
                litCreatePlanMsg.Text = string.Format(T("{0} does not have a study plan yet.", "{0} belum mempunyai pelan belajar."), _selectedChildName);
                litEditSub.Text = _selectedChildName;
                return;
            }
            _planId = planId;
            pnlContent.Visible = true;
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand("SELECT planTitle, endDate FROM dbo.StudyPlan WHERE studyPlanId=@id", c))
                {
                    ...
                    using (var r = cmd.ExecuteReader())
                    {
                        if (r.Read())
                        {
                            txtPlanTitle.Text = r["planTitle"] != DBNull.Value ? r["planTitle"].ToString() : "";
                            if (r["endDate"] != DBNull.Value)
                                txtDueDate.Text = Convert.ToDateTime(r["endDate"]).ToString("yyyy-MM-dd");
                        }
                    }
                }
            }
        ...
        private string GetActivePlanId()
        {
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("SELECT TOP 1 studyPlanId FROM dbo.StudyPlan WHERE studentParentId=@sp AND status='Ongoing' ORDER BY createdAt DESC", c))
                {
        ...
        protected void BtnCreatePlan_Click(object sender, EventArgs e)
        {
                ...
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    using (var txn = c.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var cmd = new SqlCommand(@"INSERT INTO dbo.StudyPlan(studyPlanId,studentParentId,createdByUserId,
                            planTitle,startDate,endDate,status,createdAt) VALUES(@id,@sp,@uid,@t,@s,@e,'Ongoing',@now)", c, txn))
                            {
                                cmd.Parameters.AddWithValue("@id", id);
                                cmd.Parameters.AddWithValue("@sp", _studentParentId);
                                cmd.Parameters.AddWithValue("@uid", _parentUserId);
                                cmd.Parameters.AddWithValue("@t", T("My Learning Mission", "Misi Pembelajaran Saya"));
                                cmd.Parameters.AddWithValue("@s", DateTime.Today);
                                cmd.Parameters.AddWithValue("@e", DateTime.Today.AddDays(30));
            ...
            {
                ShowMsg(T("Error creating plan.", "Ralat membuat pelan."), false);
            }
            LoadPage();
        }
        ...
        protected void BtnSaveTitle_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowMsg(T("Due date cannot be empty.", "Tarikh akhir tidak boleh kosong."), false);
                return;
            }
            if (!DateTime.TryParse(txtDueDate.Text, out dueDate))
            {
                ShowMsg(T("Invalid date format.", "Format tarikh tidak sah."), false);
                return;
            }
            if (dueDate.Date < DateTime.Today)
            {
                ShowMsg(T("Due date cannot be earlier than today.", "Tarikh akhir tidak boleh lebih awal daripada hari ini."), false);
                return;
            }
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("UPDATE dbo.StudyPlan SET planTitle=@t, endDate=@d WHERE studyPlanId=@id", c))
                {
                    cmd.Parameters.AddWithValue("@t", txtPlanTitle.Text.Trim());
                    cmd.Parameters.AddWithValue("@d", dueDate.Date);
                    cmd.Parameters.AddWithValue("@id", _planId);
                    c.Open();
                    cmd.ExecuteNonQuery();
                }
                ShowMsg(T("Plan details saved!", "Butiran pelan disimpan!"), true);
            }
            catch
            {
                ShowMsg(T("Error saving plan details.", "Ralat menyimpan butiran pelan."), false);
            }
        }
        ...
        private void LoadTaskList()
        {
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("SELECT spTaskId, taskTitle, suggestedAction, orderNo, isCompleted FROM dbo.SPTask WHERE studyPlanId=@id ORDER BY orderNo", c))
                {
                    cmd.Parameters.AddWithValue("@id", _planId);
                    c.Open();
                    using (var r = cmd.ExecuteReader())
                    {
                        while (r.Read())
                        {
                            ...
                            {
                                Id = r["spTaskId"].ToString(),
                                Title = r["taskTitle"].ToString(),
                                Action = r["suggestedAction"] != DBNull.Value ? r["suggestedAction"].ToString() : "",
                                Order = Convert.ToInt32(r["orderNo"]),
                                Done = r["isCompleted"] != DBNull.Value && Convert.ToBoolean(r["isCompleted"])
                            });
                        }
                    }
                }
        ...
        {
            litTaskFormTitle.Text = T("Add Task", "Tambah Tugasan");
            hidEditTaskId.Value = "";
            txtTaskTitle.Text = "";
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "ScrollToAddTask", "setTimeout(function(){ var el=document.getElementById('" + pnlTaskForm.ClientID + "'); if(el){el.scrollIntoView({behavior:'smooth',block:'start'});} },120);", true);
        }
        ...
        protected void BtnEditTaskTrigger_Click(object sender, EventArgs e)
        {
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("SELECT taskTitle, suggestedAction FROM dbo.SPTask WHERE spTaskId=@id AND studyPlanId=@pid", c))
                {
                    cmd.Parameters.AddWithValue("@id", taskId);
                    cmd.Parameters.AddWithValue("@pid", _planId);
                    c.Open();
                    using (var r = cmd.ExecuteReader())
                    {
                        if (r.Read())
                        {
                            txtTaskTitle.Text = r["taskTitle"].ToString();
                            txtSuggestedAction.Text = r["suggestedAction"] != DBNull.Value ? r["suggestedAction"].ToString() : "";
                        }
                    }
                }
            }
            catch { }
            hidEditTaskId.Value = taskId;
            litTaskFormTitle.Text = T("Edit Task", "Edit Tugasan");
            pnlTaskForm.Visible = true;
            LoadTaskList();
            LoadRewardList();
            BuildPreviewMarkers();
            MaintainScrollPositionOnPostBack = false;
            ScriptManager.RegisterStartupScript(this, GetType(), "ScrollToEditTask", "setTimeout(function(){ var el=document.getElementById('" + pnlTaskForm.ClientID + "'); if(el){el.scrollIntoView({behavior:'smooth',block:'start'});} },120);", true);
        }
        ...
        protected void BtnSaveTask_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowMsg(T("Task title cannot be empty.", "Tajuk tugasan tidak boleh kosong."), false);
                return;
            }
            ...
            try
            {
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    using (var txn = c.BeginTransaction())
                    {
                        try
                        {
                            ...
                            {
                                int maxOrder = 0;
                                using (var cmd = new SqlCommand("SELECT ISNULL(MAX(orderNo),0) FROM dbo.SPTask WHERE studyPlanId=@id", c, txn))
                                {
                                    cmd.Parameters.AddWithValue("@id", _planId);
                                ...
                                using (var cmd = new SqlCommand("INSERT INTO dbo.SPTask(spTaskId,studyPlanId,taskTitle,suggestedAction,orderNo,isCompleted," +
                                    "completedAt) VALUES(@id,@pid,@t,@a,@o,0,NULL)", c, txn))
                                {
                                    cmd.Parameters.AddWithValue("@id", newId);
                                    cmd.Parameters.AddWithValue("@pid", _planId);
                                    cmd.Parameters.AddWithValue("@t", taskTitle);
                                    cmd.Parameters.AddWithValue("@a", txtSuggestedAction.Text.Trim());
                                    cmd.Parameters.AddWithValue("@o", maxOrder + 1);
                                    cmd.ExecuteNonQuery();
                                }
                ...
                pnlTaskForm.Visible = false;
                ShowMsg(T("Task saved!", "Tugasan disimpan!"), true);
            }
            catch
            {
                ShowMsg(T("Error saving task.", "Ralat menyimpan tugasan."), false);
            }
            LoadTaskList();
            LoadRewardList();
            BuildPreviewMarkers();
            ...
            try
            {
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    using (var txn = c.BeginTransaction())
                    {
                        try
                        {
                            using (var cmd = new SqlCommand("DELETE FROM dbo.SPTask WHERE spTaskId=@id AND studyPlanId=@pid", c, txn))
                            {
                                cmd.Parameters.AddWithValue("@id", taskId);
                                cmd.Parameters.AddWithValue("@pid", _planId);
                                cmd.ExecuteNonQuery();
                            ...
                            int order = 1;
                            using (var cmd = new SqlCommand("SELECT spTaskId FROM dbo.SPTask WHERE studyPlanId=@pid ORDER BY orderNo", c, txn))
                            {
                                cmd.Parameters.AddWithValue("@pid", _planId);
                                using (var r = cmd.ExecuteReader())
                                {
                                    var ids = new List<string>();
                                    while (r.Read()) ids.Add(r["spTaskId"].ToString());
                                    r.Close();

                                    foreach (var id in ids)
                                    {
                                        using (var u = new SqlCommand("UPDATE dbo.SPTask SET orderNo=@o WHERE spTaskId=@id", c, txn))
                                        {
                                            u.Parameters.AddWithValue("@o", order++);
                ...
                ShowMsg(T("Task deleted.", "Tugasan dipadam."), true);
            }
            catch
            {
                ShowMsg(T("Error deleting task.", "Ralat memadam tugasan."), false);
            }
            LoadTaskList();
            LoadRewardList();
            BuildPreviewMarkers();
        ...
        {
            string taskId = hidMoveTaskId.Value;
            string dir = hidMoveDir.Value;
            try
            {
                using (var c = new SqlConnection(ConnStr))
                {
                    c.Open();
                    int curOrder = 0;
                    using (var cmd = new SqlCommand("SELECT orderNo FROM dbo.SPTask WHERE spTaskId=@id", c))
                    {
                        cmd.Parameters.AddWithValue("@id", taskId);
                    ...
                    using (var cmd = new SqlCommand("UPDATE dbo.SPTask SET orderNo=@new WHERE spTaskId=@id", c))
                    {
                        cmd.Parameters.AddWithValue("@new", swapOrder);
                        cmd.Parameters.AddWithValue("@id", taskId);
                        cmd.ExecuteNonQuery();
                    }

                    using (var cmd = new SqlCommand("UPDATE dbo.SPTask SET orderNo=@new WHERE studyPlanId=@pid AND orderNo=@swap AND spTaskId<>@id", c))
                    {
                        cmd.Parameters.AddWithValue("@new", curOrder);
                        cmd.Parameters.AddWithValue("@pid", _planId);
                        cmd.Parameters.AddWithValue("@swap", swapOrder);
                        cmd.Parameters.AddWithValue("@id", taskId);
                        cmd.ExecuteNonQuery();
                    }
                }
        ...
        private void LoadRewardList()
        {
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("SELECT rewardId, rewardName, requiredProgress, rewardImage FROM dbo.SPReward WHERE studyPlanId=@id ORDER BY requiredProgress", c))
                {
                    cmd.Parameters.AddWithValue("@id", _planId);
                    c.Open();
                    using (var r = cmd.ExecuteReader())
                    {
                        ...
                        while (r.Read())
                        {
                            ...
                            string img = r["rewardImage"] != DBNull.Value ? r["rewardImage"].ToString() : "";
                            int pct = r["requiredProgress"] != DBNull.Value ? Convert.ToInt32(r["requiredProgress"]) : 0;
                            string imgUrl = !string.IsNullOrEmpty(img) ? ResolveUrl("~/Images/Rewards/" + img) : "";
                        ...
                        <button type=""button"" class=""pt-btn-icon pt-btn-icon-danger"" onclick=""event.stopPropagation();showDeleteModal('reward','{4}','{6}','{7}');return false;""><i class=""bi bi-trash""></i></button>
                                ...
                                !string.IsNullOrEmpty(imgUrl) ? "<img src='" + imgUrl + "' alt='" + Server.HtmlEncode(name) + "'/>" : "<i class='bi bi-gift-fill'></i>",
                                Server.HtmlEncode(name), pctLabel,
                        ...
                        pnlRewardList.Controls.Add(new LiteralControl(sb.ToString()));
                    }
                }
            }
            catch { }
        ...
        protected void BtnImageSelect_Click(object sender, EventArgs e)
        {
            hidSelectedImage.Value = hidImageSelect.Value;
            LoadTaskList();
            LoadRewardList();
            BuildImageGrid();
            BuildPreviewMarkers();
        ...
        private void BuildPreviewMarkers()
        {
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("SELECT rewardName, requiredProgress, rewardImage FROM dbo.SPReward WHERE studyPlanId=@id ORDER BY requiredProgress", c))
                {
                    cmd.Parameters.AddWithValue("@id", _planId);
                    c.Open();
                    StringBuilder sb = new StringBuilder();
                    using (var r = cmd.ExecuteReader())
                    {
                        while (r.Read())
                        {
                            ...
                            int pct = r["requiredProgress"] != DBNull.Value ? Convert.ToInt32(r["requiredProgress"]) : 0;
                            string imgUrl = !string.IsNullOrEmpty(img) ? ResolveUrl("~/Images/Rewards/" + img) : "";
                            int left = Math.Max(2, Math.Min(98, pct));

                            sb.AppendFormat(@"<div class=""pt-reward-marker pt-reward-marker-unlocked"" style=""left:{0}%;"" title=""{1} ({2}%)"">{3}</div>",
                                left, Server.HtmlEncode(name), pct,
                                !string.IsNullOrEmpty(imgUrl) ? "<img src='" + imgUrl + "'/>" : "<i class='bi bi-gift-fill'></i>");
                        }
                    }
                    pnlPreviewMarkers.Controls.Add(new LiteralControl(sb.ToString()));
                }
            }
            catch { }
        }

        protected void BtnShowAddReward_Click(object sender, EventArgs e)
        {
            litRewardFormTitle.Text = T("Add Reward", "Tambah Ganjaran");
            hidEditRewardId.Value = "";
            txtRewardName.Text = "";
            txtUnlockPct.Text = "";
            hidSelectedImage.Value = "";
            pnlRewardForm.Visible = true;
            LoadTaskList();
            LoadRewardList();
            BuildImageGrid();
            BuildPreviewMarkers();
            MaintainScrollPositionOnPostBack = false;
            ScriptManager.RegisterStartupScript(this, GetType(), "ScrollToAddReward", "setTimeout(function(){ var el=document.getElementById('" + pnlRewardForm.ClientID + "'); if(el){el.scrollIntoView({behavior:'smooth',block:'start'});} },120);", true);
        }

        protected void BtnCancelReward_Click(object sender, EventArgs e)
        {
            pnlRewardForm.Visible = false;
            LoadTaskList();
            LoadRewardList();
            BuildImageGrid();
            BuildPreviewMarkers();
        ...
        protected void BtnEditRewardTrigger_Click(object sender, EventArgs e)
        {
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("SELECT rewardName, requiredProgress, rewardImage FROM dbo.SPReward WHERE rewardId=@id AND studyPlanId=@pid", c))
                {
                    ...
                    using (var r = cmd.ExecuteReader())
                    {
                        if (r.Read())
                        {
                            txtRewardName.Text = r["rewardName"] != DBNull.Value ? r["rewardName"].ToString() : "";
                            hidSelectedImage.Value = r["rewardImage"] != DBNull.Value ? r["rewardImage"].ToString() : "";
                            txtUnlockPct.Text = r["requiredProgress"] != DBNull.Value ? r["requiredProgress"].ToString() : "";
                        }
                    }
                }
            }
            ...
            hidEditRewardId.Value = rid;
            litRewardFormTitle.Text = T("Edit Reward", "Edit Ganjaran");
            pnlRewardForm.Visible = true;
            LoadTaskList();
            LoadRewardList();
            BuildImageGrid();
            BuildPreviewMarkers();
            MaintainScrollPositionOnPostBack = false;
            ScriptManager.RegisterStartupScript(this, GetType(), "ScrollToEditReward", "setTimeout(function(){ var el=document.getElementById('" + pnlRewardForm.ClientID + "'); if(el){el.scrollIntoView({behavior:'smooth',block:'start'});} },120);", true);
        }

        protected void BtnSaveReward_Click(object sender, EventArgs e)
        {
            string img = hidSelectedImage.Value;
            string name = txtRewardName.Text.Trim();
            int pct = 0;

            if (string.IsNullOrEmpty(img))
            {
                ShowMsg(T("Please select a reward image.", "Sila pilih imej ganjaran."), false);
                return;
            }
            if (string.IsNullOrEmpty(name))
            {
                ShowMsg(T("Reward name cannot be empty.", "Nama ganjaran tidak boleh kosong."), false);
                return;
            }
            if (!int.TryParse(txtUnlockPct.Text, out pct) || pct < 1 || pct > 100)
            {
                ShowMsg(T("Unlock percentage must be between 1 and 100.", "Peratus buka mesti antara 1 dan 100."), false);
                return;
            }
            ...
            try
            {
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    {
                        chk.Parameters.AddWithValue("@pid", _planId);
                        chk.Parameters.AddWithValue("@p", pct);
                        chk.Parameters.AddWithValue("@rid", string.IsNullOrEmpty(editId) ? "NONE" : editId);
                        if ((int)chk.ExecuteScalar() > 0)
                        {
                            ShowMsg(T("A reward already exists at this unlock percentage.", "Ganjaran sudah wujud pada peratus buka ini."), false);
                            return;
                        }
                    }
                    ...
                    using (var txn = c.BeginTransaction())
                    {
                        try
                        {
                            ...
                            {
                                int cnt = 0;
                                using (var cmd = new SqlCommand("SELECT COUNT(*) FROM dbo.SPReward WHERE studyPlanId=@pid", c, txn))
                                {
                                    cmd.Parameters.AddWithValue("@pid", _planId);
                                ...
                                {
                                    txn.Rollback();
                                    ShowMsg(T("Maximum 5 rewards allowed.", "Maksimum 5 ganjaran dibenarkan."), false);
                                    return;
                                }
                                ...
                                using (var cmd = new SqlCommand("INSERT INTO dbo.SPReward(rewardId,studyPlanId,rewardName,requiredProgress,isUnlocked,unlockedAt,rewardImage) VALUES(@id,@pid,@n,@p,0,NULL,@img)", c, txn))
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@img", string.IsNullOrEmpty(img) ? (object)DBNull.Value : img);
                                    cmd.ExecuteNonQuery();
                                }
                            }
                            else
                ...
                pnlRewardForm.Visible = false;
                ShowMsg(T("Reward saved!", "Ganjaran disimpan!"), true);
            }
            catch
            {
                ShowMsg(T("Error saving reward.", "Ralat menyimpan ganjaran."), false);
            }
            LoadTaskList();
            LoadRewardList();
            BuildImageGrid();
        ...
        protected void BtnDeleteReward_Click(object sender, EventArgs e)
        {
            ...
            {
                using (var c = new SqlConnection(ConnStr))
                using (var cmd = new SqlCommand("DELETE FROM dbo.SPReward WHERE rewardId=@id AND studyPlanId=@pid", c))
                {
                    cmd.Parameters.AddWithValue("@id", rid);
                    cmd.Parameters.AddWithValue("@pid", _planId);
                    c.Open();
                ...
                ShowMsg(T("Reward deleted.", "Ganjaran dipadam."), true);
            }
            catch
            {
                ShowMsg(T("Error.", "Ralat."), false);
            }
            LoadTaskList();
            LoadRewardList();
            BuildImageGrid();
