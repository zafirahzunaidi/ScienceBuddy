    ...
    public partial class ForumThread : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                btnDeleteThread.Text = T("Confirm Delete", "Sahkan Padam");
                LoadThread();
            }
            else
            {
                ...
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    using (var cmd = new SqlCommand("SELECT t.tagName FROM dbo.ForumTag ft INNER JOIN dbo.Tag t ON ft.tagId=t.tagId WHERE ft.forumId=@id", c))
                    {
                        ...
                        using (var r = cmd.ExecuteReader())
                        {
                            ...
                            if (sb.Length > 0)
                                pnlTags.Controls.Add(new LiteralControl(sb.ToString()));
                        }
                    }
                    ...
                    using (var cmd = new SqlCommand("SELECT COUNT(*) FROM dbo.ForumLike WHERE forumId=@id", c))
                    {
                        cmd.Parameters.AddWithValue("@id", _forumId);
                        likeCount = (int)cmd.ExecuteScalar();
                    }
                    using (var cmd = new SqlCommand("SELECT COUNT(*) FROM dbo.ForumLike WHERE forumId=@id AND senderUserId=@uid", c))
                    {
                        cmd.Parameters.AddWithValue("@id", _forumId);
                        cmd.Parameters.AddWithValue("@uid", _userId);
                        userLiked = (int)cmd.ExecuteScalar() > 0;
                    }
                    ...
                    // Replies
                    int replyCount = 0;
                    using (var cmd = new SqlCommand("SELECT COUNT(*) FROM dbo.ForumChat WHERE forumId=@id", c))
                    {
                        cmd.Parameters.AddWithValue("@id", _forumId);
                        replyCount = (int)cmd.ExecuteScalar();
                    }
                    ...
                    using (var cmd = new SqlCommand("SELECT fc.message, fc.createdAt, u.username FROM dbo.ForumChat fc INNER JOIN dbo.[User] u ON fc.senderUserId=u.userId WHERE fc.forumId=@id ORDER BY fc.createdAt ASC", c))
                    {
                        ...
                        using (var r = cmd.ExecuteReader())
                        {
                            ...
                            {
                                has = true;
                                string postedText = T("Posted ", "Dihantar pada ") + GetTimeAgo(Convert.ToDateTime(r["createdAt"]));
                                sb.AppendFormat("<div class='pt-forum-reply-card'><div class='pt-forum-reply-avatar'>{0}</div><div class='pt-forum-reply-content'><div class='pt-forum-reply-meta'><span class='pt-forum-reply-author'>{1}</span> <span class='pt-forum-reply-date'>&bull; {2}</span></div><div class='pt-forum-reply-message'>{3}</div></div></div>",
                                    r["username"].ToString().Substring(0, 1).ToUpper(),
                                    Server.HtmlEncode(r["username"].ToString()),
                                    postedText,
                                    Server.HtmlEncode(r["message"].ToString()).Replace("\n", "<br/>"));
                            }
                            if (has)
                            {
                                pnlReplies.Controls.Add(new LiteralControl(sb.ToString()));
                    ...
                    if (createdBy == _userId)
                    {
                        ...
                        lnkEdit.HRef = "EditDiscussion.aspx?forumId=" + _forumId;
                    }
                    else
                    {
        ...
        protected void BtnLike_Click(object sender, EventArgs e)
        {
            try
            {
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("@fid", _forumId);
                        cmd.Parameters.AddWithValue("@uid", _userId);
                        var r = cmd.ExecuteScalar();
                        if (r != null && r != DBNull.Value) liked = true;
                    }

                    if (liked)
                    {
                        using (var cmd = new SqlCommand("DELETE FROM dbo.ForumLike WHERE forumId=@fid AND senderUserId=@uid", c))
                        {
                            cmd.Parameters.AddWithValue("@fid", _forumId);
                            cmd.Parameters.AddWithValue("@uid", _userId);
                            cmd.ExecuteNonQuery();
                        ...
                        using (var txn = c.BeginTransaction())
                        {
                            try
                            {
                                ...
                                if (!string.IsNullOrEmpty(authorId) && authorId != _userId)
                                {
                                    ...
                                    string threadTitle = "";
                                    using (var cmd = new SqlCommand("SELECT title FROM dbo.Forum WHERE forumId=@id", c, txn))
                                    {
                                        cmd.Parameters.AddWithValue("@id", _forumId);
                                        threadTitle = cmd.ExecuteScalar()?.ToString() ?? "";
                                    }
                                    ...
                                    using (var cmd = new SqlCommand("INSERT INTO dbo.Notification(notificationId,toUserId,titleEN,titleBM,messageEN,messageBM,isRead,createdAt) VALUES(@id,@to,@te,@tb,@me,@mb,0,@now)", c, txn))
                                    {
                                        ...
                                        cmd.Parameters.AddWithValue("@to", authorId);
                                        cmd.Parameters.AddWithValue("@te", "Your parent liked your discussion");
                                        cmd.Parameters.AddWithValue("@tb", "Ibu bapa anda menyukai perbincangan anda");
                                        cmd.Parameters.AddWithValue("@me", username + " (parent) liked your discussion: " + threadTitle + ".");
                                        cmd.Parameters.AddWithValue("@mb", username + " (ibu bapa) menyukai perbincangan anda: " + threadTitle + ".");
                                        cmd.Parameters.AddWithValue("@now", DateTime.Now);
                                        cmd.ExecuteNonQuery();
                                    }
                                }
        ...
        protected void BtnReply_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowMsg(T("Reply cannot be empty.", "Balasan tidak boleh kosong."), false);
                return;
            }
                        ...
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(authorId) && authorId != _userId)
                            {
                                ...
                                using (var cmd = new SqlCommand("INSERT INTO dbo.Notification(notificationId,toUserId,titleEN,titleBM,messageEN,messageBM,isRead,createdAt) VALUES(@id,@to,@te,@tb,@me,@mb,0,@now)", c, txn))
                                {
                                    cmd.Parameters.AddWithValue("@id", nid);
                                    cmd.Parameters.AddWithValue("@to", authorId);
                                    cmd.Parameters.AddWithValue("@te", "Your parent replied to your discussion");
                                    cmd.Parameters.AddWithValue("@tb", "Ibu bapa anda membalas perbincangan anda");
                                    cmd.Parameters.AddWithValue("@me", username + " (parent) replied to your discussion: " + threadTitle + ".");
                                    cmd.Parameters.AddWithValue("@mb", username + " (ibu bapa) membalas perbincangan anda: " + threadTitle + ".");
                                    cmd.Parameters.AddWithValue("@now", DateTime.Now);
                                    cmd.ExecuteNonQuery();
                                }
                            }
            ...
            {
                ShowMsg(T("Error posting reply.", "Ralat menghantar balasan."), false);
            }
            LoadThread();
        }
                ...
                using (var c = new SqlConnection(ConnStr))
                {
                    ...
                    using (var txn = c.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var cmd = new SqlCommand("DELETE FROM dbo.ForumLike WHERE forumId=@id", c, txn))
                            {
                                cmd.Parameters.AddWithValue("@id", _forumId);
                                cmd.ExecuteNonQuery();
                            }
                            using (var cmd = new SqlCommand("DELETE FROM dbo.ForumTag WHERE forumId=@id", c, txn))
                            {
                                cmd.Parameters.AddWithValue("@id", _forumId);
                                cmd.ExecuteNonQuery();
                            }
                            using (var cmd = new SqlCommand("DELETE FROM dbo.ForumChat WHERE forumId=@id", c, txn))
                            {
                                cmd.Parameters.AddWithValue("@id", _forumId);
                                cmd.ExecuteNonQuery();
                            }
                            using (var cmd = new SqlCommand("DELETE FROM dbo.Forum WHERE forumId=@id", c, txn))
                            {
                                cmd.Parameters.AddWithValue("@id", _forumId);
                                cmd.ExecuteNonQuery();
                            }
                            ...
                            txn.Commit();
                            Response.Redirect("Forum.aspx", false);
                            Context.ApplicationInstance.CompleteRequest();
                        }
                        catch
                        {
        ...
        private void LoadSidebarChildren()
        {
            try
            {
                ...
                {
                    string pid = "";
                    using (var cmd = new System.Data.SqlClient.SqlCommand("SELECT parentId FROM dbo.[Parent] WHERE userId=@u", c))
                    {
                    ...
                    using (var cmd = new System.Data.SqlClient.SqlCommand("SELECT s.studentId, ISNULL(s.nickname,s.name) AS n FROM dbo.StudentParent sp INNER JOIN dbo.Student s ON sp.studentId=s.studentId WHERE sp.parentId=@p ORDER BY s.name", c))
                    {
                        cmd.Parameters.AddWithValue("@p", pid);
                        using (var r = cmd.ExecuteReader())
                        {
                            while (r.Read())
                                ddlSidebarChild2.Items.Add(new System.Web.UI.WebControls.ListItem(r["n"].ToString(), r["studentId"].ToString()));
                        }
                    }
                }
            }
            ...
            if (!string.IsNullOrEmpty(saved) && ddlSidebarChild2.Items.FindByValue(saved) != null)
                ddlSidebarChild2.SelectedValue = saved;
        }
    }
}