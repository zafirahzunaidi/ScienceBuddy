                    ...
                    else
                    {
                        using (var txn = c.BeginTransaction()) { try {
                            ...
                            if (!string.IsNullOrEmpty(authorId) && authorId != _userId)
                            {
                                ...
                                { cmd.Parameters.AddWithValue("@id", nid); cmd.Parameters.AddWithValue("@to", authorId); cmd.Parameters.AddWithValue("@te", "Your parent liked your discussion"); cmd.Parameters.AddWithValue("@tb", "Ibu bapa anda menyukai perbincangan anda"); cmd.Parameters.AddWithValue("@me", username + " (parent) liked your discussion: " + threadTitle + "."); cmd.Parameters.AddWithValue("@mb", username + " (ibu bapa) menyukai perbincangan anda: " + threadTitle + "."); cmd.Parameters.AddWithValue("@now", DateTime.Now); cmd.ExecuteNonQuery(); }
                            }
                            txn.Commit();
                        } catch { txn.Rollback(); } }
                    }
            ...
            try
            {
                using (var c = new SqlConnection(ConnStr)) { c.Open(); using (var txn = c.BeginTransaction()) { try {
                    ...
                    if (!string.IsNullOrEmpty(authorId) && authorId != _userId)
                    {
                        ...
                        string username = ""; using (var cmd = new SqlCommand("SELECT username FROM dbo.[User] WHERE userId=@u", c, txn)) { cmd.Parameters.AddWithValue("@u", _userId); username = cmd.ExecuteScalar()?.ToString() ?? ""; }
                        using (var cmd = new SqlCommand("INSERT INTO dbo.Notification(notificationId,toUserId,titleEN,titleBM,messageEN,messageBM,isRead,createdAt) VALUES(@id,@to,@te,@tb,@me,@mb,0,@now)", c, txn))
                        { cmd.Parameters.AddWithValue("@id", nid); cmd.Parameters.AddWithValue("@to", authorId); cmd.Parameters.AddWithValue("@te", "Your parent replied to your discussion"); cmd.Parameters.AddWithValue("@tb", "Ibu bapa anda membalas perbincangan anda"); cmd.Parameters.AddWithValue("@me", username + " (parent) replied to your discussion: " + threadTitle + "."); cmd.Parameters.AddWithValue("@mb", username + " (ibu bapa) membalas perbincangan anda: " + threadTitle + "."); cmd.Parameters.AddWithValue("@now", DateTime.Now); cmd.ExecuteNonQuery(); }
                    }
