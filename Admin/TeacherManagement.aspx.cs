                ...
                using (var conn = new SqlConnection(ConnStr))
                {
                    ...
                    using (var txn = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            // 3. Generate TeacherID
                            string teacherId = GenId(conn, "Teacher", "teacherId", "T", txn);

                            // 4. Insert Teacher (foreign key to User)
                            using (var cmd = new SqlCommand("INSERT INTO dbo.[Teacher]([teacherId],[userId],[name],[phoneNumber],[academicQualification],[status],[approvedDate]) VALUES(@tid,@uid,@name,@ph,@qual,'Pending',NULL)", conn, txn))
