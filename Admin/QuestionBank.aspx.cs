    ...
    public partial class QuestionBank : Page
    {
        ...
        private void SetUserInfo()
        {
            using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                ...
                { cmd.Parameters.AddWithValue("@u", Session["userId"].ToString());
                    var v = cmd.ExecuteScalar(); string name = v?.ToString() ?? "Admin";
        ...
        private void LoadStats()
        {
            using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                ...
                int total = int.Parse(litTotal.Text); int mcq = int.Parse(litMCQ.Text); int tf = int.Parse(litTF.Text);
        ...
        private void LoadQuestions(string search, string type, string diff, string status)
        {
            ...
            using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                ...
                if (!string.IsNullOrWhiteSpace(search)) sql += " AND (q.[questionTextEN] LIKE @s OR q.[questionTextBM] LIKE @s OR ISNULL(t.[name],'') LIKE @s OR u.[username] LIKE @s OR q.[questionId] LIKE @s OR ISNULL(st.[subtopicTitleEN],'') LIKE @s)";
                ...
                using (var cmd = new SqlCommand(sql, conn))
                {
                    ...
                    using (var da = new SqlDataAdapter(cmd)) {
                        ...
                        {
                            string textEN = NS(r, "questionTextEN"); string textBM = NS(r, "questionTextBM");
                            ...
                            list.Add(new {
                                questionId = NS(r, "questionId"), questionText = text.Length > 150 ? text.Substring(0, 150) + "..." : text,
                                questionType = NS(r, "questionType"), difficulty = NS(r, "difficulty"), status = NS(r, "status"),
                                teacherName = NS(r, "teacherName"), subtopicName = sub, language = NS(r, "status"),
                                createdAt = r["createdAt"] != DBNull.Value ? Convert.ToDateTime(r["createdAt"]).ToString("d MMM yyyy") : "-",
                                typeCls = GetTypeCls(NS(r, "questionType")), diffCls = GetDiffCls(NS(r, "difficulty")), jsonData = json
                            });
                        }
                    }
                }
            }
            if (list.Count > 0) { rptQuestions.DataSource = list; rptQuestions.DataBind(); pnlEmpty.Visible = false; }
            else { pnlEmpty.Visible = true; }
        }

        protected void btnSearch_Click(object sender, EventArgs e) { LoadStats(); LoadQuestions(txtSearch.Text.Trim(), ddlType.SelectedValue, ddlDiff.SelectedValue, ddlStatus.SelectedValue); }
        protected void btnReset_Click(object sender, EventArgs e) { txtSearch.Text = ""; ddlType.SelectedIndex = 0; ddlDiff.SelectedIndex = 0; ddlStatus.SelectedIndex = 0; LoadStats(); LoadQuestions("", "", "", ""); }

        // AJAX: Get question for edit
        ...
        private void HandleSaveQuestion()
        {
            ...
            try {
                ...
                using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                    ...
                    {
                        cmd.Parameters.AddWithValue("@tEN", textEN); cmd.Parameters.AddWithValue("@tBM", textBM);
                        cmd.Parameters.AddWithValue("@oA", optA); cmd.Parameters.AddWithValue("@oB", optB);
                        cmd.Parameters.AddWithValue("@oC", optC); cmd.Parameters.AddWithValue("@oD", optD);
                        cmd.Parameters.AddWithValue("@cor", correct); cmd.Parameters.AddWithValue("@dif", diff);
                        cmd.Parameters.AddWithValue("@id", qId); cmd.ExecuteNonQuery();
                    }
        ...
        private void InsertLog(SqlConnection c, string uid, string action, string desc, string status)
        {
            ...
            { cmd.Parameters.AddWithValue("@a", id); cmd.Parameters.AddWithValue("@b", uid); cmd.Parameters.AddWithValue("@c", action); cmd.Parameters.AddWithValue("@d", desc); cmd.Parameters.AddWithValue("@e", DateTime.Now); cmd.Parameters.AddWithValue("@f", status); cmd.ExecuteNonQuery(); }
        }
        ...
        private string GetTypeCls(string t) { switch ((t ?? "").ToLower()) { case "mcq": return "qb-bdg-mcq"; case "true/false": return "qb-bdg-tf"; case "multi select": return "qb-bdg-multi"; case "drag & drop": return "qb-bdg-drag"; case "fill in the blank": return "qb-bdg-fill"; default: return "qb-bdg-other"; } }
        private string GetDiffCls(string d) { switch ((d ?? "").ToLower()) { case "easy": return "qb-bdg-easy"; case "medium": return "qb-bdg-medium"; case "hard": return "qb-bdg-hard"; default: return "qb-bdg-other"; } }
        ...
        private static string JStr(string key, string val) { return "\"" + key + "\":\"" + EJ(val) + "\""; }
        private static string EJ(object v) { string s = (v == null || v == DBNull.Value) ? "" : v.ToString(); return s.Replace("\\", "\\\\").Replace("\"", "\\\"").Replace("\n", "\\n").Replace("\r", ""); }
    }
}
