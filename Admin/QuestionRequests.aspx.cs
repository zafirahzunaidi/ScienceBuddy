...
namespace ScienceBuddy.Admin
{
    public partial class QuestionRequests : Page
    {
        ...
        {
            if (Request.QueryString["handler"] == "ReviewQuiz" && Request.HttpMethod == "POST") { _isAjax = true; HandleReview(); return; }
            if (Request.QueryString["handler"] == "ViewQuiz" && Request.HttpMethod == "POST") { _isAjax = true; HandleView(); return; }

            if (Session["userId"] == null || Session["role"]?.ToString() != "Admin")
            { Response.Redirect("~/Login.aspx", false); return; }
            ((ScienceBuddy.SiteMaster)Master).LayoutMode = "Sidebar";

            if (!IsPostBack) { SetUserInfo(); LoadStats(); LoadData("", "", "", ""); }
        ...
        private void LoadStats()
        {
            using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                ...
                litBadge.Text = pCount != "0" ? "<span class=\"sb-badge sb-badge-warning\" style=\"margin-left:6px;\">" + pCount + "</span>" : "";
            }
        }
        ...
        private void LoadData(string search, string statusF, string typeF, string langF)
        {
            using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                ...
                if (!string.IsNullOrWhiteSpace(statusF)) baseSql += " AND q.[status]=@st";
                if (!string.IsNullOrWhiteSpace(typeF)) baseSql += " AND q.[quizType]=@tp";
                if (!string.IsNullOrWhiteSpace(langF)) baseSql += " AND q.[language]=@lg";
                ...
                using (var cmd = new SqlCommand(pendingSql, conn)) {
                    ...
                    if (dt.Rows.Count > 0) { pnlPending.Visible = true; pnlPendingEmpty.Visible = false;
                        var list = new List<object>();
                        foreach (DataRow r in dt.Rows) list.Add(new { quizId = r["quizId"].ToString(), quizTitle = NS(r["quizTitle"]), quizType = NS(r["quizType"]), language = NS(r["language"]), teacherName = NS(r["teacherName"]), questionCount = r["questionCount"].ToString(), createdAt = r["createdAt"] != DBNull.Value ? Convert.ToDateTime(r["createdAt"]).ToString("dd MMM yyyy") : "-" });
                        rptPending.DataSource = list; rptPending.DataBind();
                    } else { pnlPending.Visible = false; pnlPendingEmpty.Visible = true; }
                }
                ...
                using (var cmd = new SqlCommand(histSql, conn)) {
                    ...
                    if (!string.IsNullOrWhiteSpace(langF)) cmd.Parameters.AddWithValue("@lg", langF);
                    var da = new SqlDataAdapter(cmd); var dt = new DataTable(); da.Fill(dt);
                    if (dt.Rows.Count > 0) { pnlHistory.Visible = true; pnlHistoryEmpty.Visible = false;
                        var list = new List<object>();
                        foreach (DataRow r in dt.Rows) list.Add(new { quizId = r["quizId"].ToString(), quizTitle = NS(r["quizTitle"]), quizType = NS(r["quizType"]), teacherName = NS(r["teacherName"]), questionCount = r["questionCount"].ToString(), reviewedDate = r["createdAt"] != DBNull.Value ? Convert.ToDateTime(r["createdAt"]).ToString("dd MMM yyyy") : "-", status = NS(r["status"]) });
                        rptHistory.DataSource = list; rptHistory.DataBind();
                    } else { pnlHistory.Visible = false; pnlHistoryEmpty.Visible = true; }
                }
            }
        }

        protected void btnSearch_Click(object sender, EventArgs e) { LoadStats(); LoadData(txtSearch.Text.Trim(), fStatus.SelectedValue, fType.SelectedValue, fLang.SelectedValue); }
        protected void btnReset_Click(object sender, EventArgs e) { txtSearch.Text = ""; fStatus.SelectedIndex = 0; fType.SelectedIndex = 0; fLang.SelectedIndex = 0; LoadStats(); LoadData("", "", "", ""); }

        // ══ AJAX: Approve/Reject Quiz ══
        private void HandleReview()
        {
            Response.Clear(); Response.ContentType = "application/json";
            try
            {
                if (Session["userId"] == null || Session["role"]?.ToString() != "Admin") { Response.Write("{\"success\":false,\"msg\":\"Unauthorized\"}"); Flush(); return; }
                ...
                using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                    // Verify quiz is Pending
                    string curStatus = "";
                    using (var cmd = new SqlCommand("SELECT [status] FROM dbo.[Quiz] WHERE [quizId]=@id", conn)) { cmd.Parameters.AddWithValue("@id", quizId); var v = cmd.ExecuteScalar(); curStatus = v?.ToString() ?? ""; }
                    if (curStatus != "Pending") { Response.Write("{\"success\":false,\"msg\":\"Quiz already reviewed.\"}"); Flush(); return; }
                    ...
                    using (var txn = conn.BeginTransaction())
                    {
                        ...
                        {
                            // Update Quiz status
                            using (var cmd = new SqlCommand("UPDATE dbo.[Quiz] SET [status]=@st WHERE [quizId]=@id", conn, txn)) { cmd.Parameters.AddWithValue("@st", newStatus); cmd.Parameters.AddWithValue("@id", quizId); cmd.ExecuteNonQuery(); }
                            // Update all questions in the quiz
                            using (var cmd = new SqlCommand("UPDATE dbo.[Question] SET [status]=@st, [reviewedDate]=GETDATE() WHERE [quizId]=@id", conn, txn)) { cmd.Parameters.AddWithValue("@st", newStatus); cmd.Parameters.AddWithValue("@id", quizId); cmd.ExecuteNonQuery(); }
                            // Log
                            string logId = GenId(conn, txn, "Log", "logId", "LOG");
                            using (var cmd = new SqlCommand("INSERT INTO dbo.[Log]([logId],[userId],[action],[description],[logDateTime],[status]) VALUES(@a,@b,@c,@d,GETDATE(),'Success')", conn, txn)) { cmd.Parameters.AddWithValue("@a", logId); cmd.Parameters.AddWithValue("@b", adminId); cmd.Parameters.AddWithValue("@c", action == "Approve" ? "Approved Quiz" : "Rejected Quiz"); cmd.Parameters.AddWithValue("@d", "Admin " + action.ToLower() + "d quiz " + quizId + "."); cmd.ExecuteNonQuery(); }

                            // Notification to teacher
                            string teacherUserId = "";
                            using (var cmd = new SqlCommand("SELECT [createdByUserId] FROM dbo.[Quiz] WHERE [quizId]=@id", conn, txn)) { cmd.Parameters.AddWithValue("@id", quizId); var v = cmd.ExecuteScalar(); teacherUserId = v?.ToString() ?? ""; }
                            if (!string.IsNullOrEmpty(teacherUserId))
                            {
                                string tEN, tBM, mEN, mBM;
                                // Get quiz title
                                string qTitle = "";
                                using (var cmd = new SqlCommand("SELECT ISNULL([quizTitleEN],[quizTitleBM]) FROM dbo.[Quiz] WHERE [quizId]=@id", conn, txn)) { cmd.Parameters.AddWithValue("@id", quizId); var v = cmd.ExecuteScalar(); qTitle = v?.ToString() ?? quizId; }
                                ...
                                { cmd.Parameters.AddWithValue("@a", nId); cmd.Parameters.AddWithValue("@b", teacherUserId); cmd.Parameters.AddWithValue("@c", tEN); cmd.Parameters.AddWithValue("@d", tBM); cmd.Parameters.AddWithValue("@e", mEN); cmd.Parameters.AddWithValue("@f", mBM); cmd.ExecuteNonQuery(); }
                            }
                            txn.Commit();
                        }
                        catch { txn.Rollback(); Response.Write("{\"success\":false,\"msg\":\"Transaction failed.\"}"); Flush(); return; }
                    }
        ...
        private void HandleView()
        {
            ...
            try
            {
                ...
                using (var conn = new SqlConnection(ConnStr)) { conn.Open();
                    ...
                    using (var cmd = new SqlCommand(sql, conn)) { cmd.Parameters.AddWithValue("@id", quizId);
                    ...
                    using (var cmd = new SqlCommand(qSql, conn)) { cmd.Parameters.AddWithValue("@id", quizId);
                        using (var rd = cmd.ExecuteReader()) { bool first = true;
                            while (rd.Read()) { if (!first) sb.Append(","); first = false;
                                sb.Append("{");
        ...
        protected string GetTypeClass(object quizType)
        {
            ...
            switch (t) { case "Unit": return "sb-badge sb-badge-primary"; case "Practice": return "sb-badge sb-badge-warning"; case "Level": return "sb-badge sb-badge-success"; default: return "sb-badge sb-badge-gray"; }
        }

        protected string BuildBadge(string status)
        {
            switch (status) { case "Approved": return "<span class=\"sb-badge sb-badge-success\"><i class=\"bi bi-check-circle-fill\"></i> Approved</span>"; case "Rejected": return "<span class=\"sb-badge sb-badge-error\"><i class=\"bi bi-x-circle-fill\"></i> Rejected</span>"; default: return "<span class=\"sb-badge sb-badge-warning\">" + status + "</span>"; }
        }

        private string SC(SqlConnection c, string sql) { try { using (var cmd = new SqlCommand(sql, c)) { var v = cmd.ExecuteScalar(); return v != null && v != DBNull.Value ? Convert.ToInt32(v).ToString() : "0"; } } catch { return "0"; } }
        ...
        {
            try { using (var cmd = new SqlCommand(string.Format("SELECT MAX(CAST(SUBSTRING([{0}],{1},LEN([{0}])-{2}) AS INT)) FROM dbo.[{3}]", col, prefix.Length + 1, prefix.Length, table), conn, txn)) { var v = cmd.ExecuteScalar(); int next = (v != null && v != DBNull.Value) ? Convert.ToInt32(v) + 1 : 1; return prefix + next.ToString("D3"); } }
            catch { return prefix + DateTime.Now.Ticks.ToString().Substring(10); }
        }

        private void Flush() { Response.Flush(); HttpContext.Current.ApplicationInstance.CompleteRequest(); }
    }
}
