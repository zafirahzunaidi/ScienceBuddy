...
// Admin SuspiciousLogins - Code Behind
namespace ScienceBuddy.Admin
{
    public partial class SuspiciousLogins : Page
    {
        ...
        {
            LoadStats();
            LoadLogs(txtSearch.Text.Trim(), ddlStatus.SelectedValue);
            LoadSecurityActions();
        }
            ...
            using (var conn = new SqlConnection(ConnStr))
            {
                ...
                using (var cmd = new SqlCommand(sql, conn))
                {
                    ...
                    foreach (DataRow r in dt.Rows)
                    {
                        ...
                        string uStatus = NS(r["userStatus"]);
                        ...
                        list.Add(new
                        {
                            ...
                            statusLabel = st == "Failed" ? T("Failed", "Gagal")
                                : st == "Warning" ? T("Warning", "Amaran")
                                : st == "Success" ? T("Success", "Berjaya") : st,
                            statusCls = st == "Failed" ? "sb-badge-error"
