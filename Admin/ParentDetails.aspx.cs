...
// Admin ParentDetails - Code Behind
    ...
    public partial class ParentDetails : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                { Response.Redirect("~/Admin/ParentManagement.aspx", false); return; }
                LoadParent(id);
            }
        }
                    ...
                    FROM dbo.[Parent] p LEFT JOIN dbo.[User] u ON u.[userId]=p.[userId] WHERE p.[parentId]=@pid", conn))
                {
                    ...
                    using (var rd = cmd.ExecuteReader())
                    {
                        ...
                        litAccountFields.Text =
                            ...
                            Field(T("Parent ID", "ID Ibu Bapa"), parentId) +
                            Field(T("Status", "Status"), status) +
                            Field(T("Role", "Peranan"), "Parent") +
                            Field(T("Children Linked", "Anak Dihubungkan"), rd["children"].ToString());
                    }
                }
                ...
                using (var cmd = new SqlCommand("SELECT TOP 20 [action],[description],[logDateTime] FROM dbo.[Log] WHERE [userId]=@u ORDER BY [logDateTime] DESC", conn))
                {
                    ...
                    using (var rd = cmd.ExecuteReader())
                    {
                        while (rd.Read())
                        {
                            ...
                            html += "<div class='ad-parent-details-log-item'><div class='ad-parent-details-log-ico'><i class='bi bi-activity'></i></div><div class='ad-parent-details-log-body'><div class='ad-parent-details-log-action'>" +
                                HttpUtility.HtmlEncode(NS(rd["action"])) + "</div><div class='ad-parent-details-log-desc'>" +
                                HttpUtility.HtmlEncode(NS(rd["description"])) + "</div><div class='ad-parent-details-log-time'>" + dt + "</div></div></div>";
                        }
                    }
        ...
        private void HandleAjax()
        {
            ...
            try
            {
                ...
                using (var conn = new SqlConnection(ConnStr))
                {
                    conn.Open();
                    string userId = "";
                    using (var cmd = new SqlCommand("SELECT [userId] FROM dbo.[Parent] WHERE [parentId]=@p", conn))
                    {
                    ...
                    switch (action)
                    {
                        ...
                        case "edit":
                            ...
                            using (var cmd = new SqlCommand("UPDATE dbo.[Parent] SET [name]=@n,[phoneNumber]=@p WHERE [parentId]=@pid", conn))
                            {
                            ...
                            ILog(conn, adminId, "Updated Parent", "Updated " + parentId);
                        ...
                        case "changeStatus":
                            ...
                            using (var cmd = new SqlCommand("SELECT p.[name], u.[email] FROM dbo.[Parent] p LEFT JOIN dbo.[User] u ON u.[userId]=p.[userId] WHERE p.[parentId]=@p", conn))
                            {
                                cmd.Parameters.AddWithValue("@p", parentId);
                                using (var rd = cmd.ExecuteReader())
                                {
                            ...
                            Response.Write("{\"success\":true,\"emailSent\":\"" + EJ(csEmail) + "\",\"emailStatus\":\"" + EJ(csStatus) + "\"}");
                            break;
                        ...
                        case "archive":
                            ...
                            ILog(conn, adminId, "Archived Parent", parentId);
                            Response.Write("{\"success\":true}");
                            break;
                        ...
                        case "resetPw":
                            ...
                            ILog(conn, adminId, "Password Reset", "Reset for " + parentId);
        ...
        {
            return "<div class='ad-parent-details-field'><div class='ad-parent-details-field-label'>" +
                HttpUtility.HtmlEncode(label) + "</div><div class='ad-parent-details-field-value'>" +
