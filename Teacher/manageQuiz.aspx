...
<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1rem;flex-wrap:wrap;gap:.75rem;">
    ...
    <asp:Panel ID="pnlCreateBtn" runat="server">
        <button type="button" class="tc-manage-quiz-btn-create" onclick="openPracticeModal()"><i class="bi bi-plus-lg"></i> <%: T("Create Quiz","Cipta Kuiz") %></button>
    </asp:Panel>
...
<div class="tc-manage-quiz-tabs">
    <asp:LinkButton ID="btnTabUnitLevel" runat="server" CssClass="tc-manage-quiz-tab active" OnClick="btnTabUnitLevel_Click" CausesValidation="false"><i class="bi bi-journal-text"></i> <%: T("Unit / Level Quizzes","Kuiz Unit / Tahap") %></asp:LinkButton>
    <asp:LinkButton ID="btnTabMine" runat="server" CssClass="tc-manage-quiz-tab" OnClick="btnTabMine_Click" CausesValidation="false"><i class="bi bi-folder2-open"></i> <%: T("Practice Quizzes","Kuiz Latihan") %></asp:LinkButton>
    <asp:LinkButton ID="btnTabDiscover" runat="server" CssClass="tc-manage-quiz-tab" OnClick="btnTabDiscover_Click" CausesValidation="false"><i class="bi bi-globe2"></i> <%: T("Discover Quizzes","Terokai Kuiz") %></asp:LinkButton>
</div>
...
<div id="ulSegFilter" class="tc-manage-quiz-seg-wrap" style="display:none;">
    <div class="tc-manage-quiz-seg">
        <button type="button" class="tc-manage-quiz-seg-btn tc-manage-quiz-seg-active" onclick="filterUL('all',this)"><%: T("All Quizzes","Semua Kuiz") %></button>
        <button type="button" class="tc-manage-quiz-seg-btn" onclick="filterUL('unit',this)"><%: T("Unit Quizzes","Kuiz Unit") %></button>
        <button type="button" class="tc-manage-quiz-seg-btn" onclick="filterUL('level',this)"><%: T("Level Quizzes","Kuiz Tahap") %></button>
    </div>
</div>

<%-- Search & Filter --%>
<div class="tc-manage-quiz-filter-bar" id="mqFilterBar">
    <div class="tc-manage-quiz-search-wrap"><i class="bi bi-search"></i>
        <asp:TextBox ID="txtSearch" runat="server" CssClass="tc-manage-quiz-search-input" /></div>
    ...
    <asp:DropDownList ID="ddlLanguage" runat="server" CssClass="tc-manage-quiz-select" AutoPostBack="true" OnSelectedIndexChanged="ddlFilter_Changed" />
    <asp:Button ID="btnSearch" runat="server" CssClass="tc-manage-quiz-btn-search" OnClick="btnSearch_Click" CausesValidation="false" />
</div>

<%-- Status chips (My Quizzes only) --%>
<asp:Panel ID="pnlStatusChips" runat="server">
<div class="tc-manage-quiz-chips">
    <asp:LinkButton ID="btnChipAll" runat="server" CssClass="tc-manage-quiz-chip active" OnClick="btnChip_Click" CommandArgument="" CausesValidation="false" data-s="all"><%: T("All","Semua") %></asp:LinkButton>
    <asp:LinkButton ID="btnChipApproved" runat="server" CssClass="tc-manage-quiz-chip" OnClick="btnChip_Click" CommandArgument="Approved" CausesValidation="false" data-s="approved"><%: T("Approved","Diluluskan") %></asp:LinkButton>
    <asp:LinkButton ID="btnChipPending" runat="server" CssClass="tc-manage-quiz-chip" OnClick="btnChip_Click" CommandArgument="Pending" CausesValidation="false" data-s="pending"><%: T("Pending","Menunggu") %></asp:LinkButton>
    <asp:LinkButton ID="btnChipRejected" runat="server" CssClass="tc-manage-quiz-chip" OnClick="btnChip_Click" CommandArgument="Rejected" CausesValidation="false" data-s="rejected"><%: T("Rejected","Ditolak") %></asp:LinkButton>
</div>
</asp:Panel>
            ...
            <ItemTemplate>
                <div class="tc-manage-quiz-dcard">
                    ...
                    <div class="tc-manage-quiz-dcard-img-wrap">
                        ...
                        <span class="tc-practice-quiz-status-pill tc-practice-quiz-status-<%# (Eval("status") ?? "Pending").ToString().ToLower() %>"><%# HttpUtility.HtmlEncode(Eval("status") ?? "Pending") %></span>
                        <span class="tc-manage-quiz-dcard-badge tc-manage-quiz-dcard-badge-left"><i class="bi bi-list-check"></i> <%# Eval("questionCount") %> Qs</span>
                        <span class="tc-manage-quiz-dcard-badge tc-manage-quiz-dcard-badge-right"><%# HttpUtility.HtmlEncode(Eval("language")?.ToString() ?? "") %></span>
                    </div>
                    <%-- Body: title only --%>
                    <div class="tc-manage-quiz-dcard-body">
                        <div class="tc-manage-quiz-dcard-title"><%# HttpUtility.HtmlEncode(Eval("quizTitle")) %></div>
                    </div>
                    <%-- Action buttons --%>
                    <div class="tc-practice-quiz-card-actions-wrap">
                        ...
                        <a href="#" class="tc-practice-quiz-action-view" onclick="openPracticeViewModal(this.closest('.tc-practice-quiz-card-actions-wrap').querySelector('.tc-practice-quiz-hidden-data'));return false;"><i class="bi bi-eye"></i> <%: T("View Quiz","Lihat Kuiz") %></a>
                        <button type="button" class="tc-practice-quiz-action-resubmit" style='<%# (Eval("status") ?? "").ToString().Equals("Rejected", StringComparison.OrdinalIgnoreCase) ? "" : "display:none;" %>' onclick="resubmitQuiz('<%# Eval("quizId") %>')"><i class="bi bi-arrow-repeat"></i> <%: T("Resubmit","Hantar Semula") %></button>
                        <button type="button" class="tc-practice-quiz-action-delete" onclick="openDeleteModal('<%# Eval("quizId") %>')"><i class="bi bi-trash"></i> <%: T("Delete Quiz","Padam Kuiz") %></button>
                    </div>
                </div>
            </ItemTemplate>
        </asp:Repeater>
...
<asp:Panel ID="pnlEmpty" runat="server" Visible="false">
    <div class="tc-manage-quiz-empty">
        ...
        <div class="tc-manage-quiz-empty-title"><asp:Literal ID="litEmptyMsg" runat="server" /></div>
    </div>
</asp:Panel>
...
<div id="pendingNoticeUL" class="tc-manage-quiz-pending-notice" style="display:none;">
    <div class="tc-manage-quiz-pending-notice-icon"><i class="bi bi-shield-exclamation"></i></div>
    <div class="tc-manage-quiz-pending-notice-content">
        <div class="tc-manage-quiz-pending-notice-title"><%: T("Verification Pending","Pengesahan Menunggu") %></div>
        ...
        <ItemTemplate>
            <div class="tc-manage-quiz-ulq-card">
                <div class="tc-manage-quiz-ulq-left">
                    <div class="tc-manage-quiz-ulq-icon tc-manage-quiz-ulq-icon-level"><i class="bi bi-trophy-fill"></i></div>
                    <div class="tc-manage-quiz-ulq-info">
                        <div class="tc-manage-quiz-ulq-title"><%# HttpUtility.HtmlEncode(Eval("levelName")) %></div>
                    </div>
                </div>
                <div class="tc-manage-quiz-ulq-stats">
                    <div class="tc-manage-quiz-ulq-col tc-manage-quiz-ulq-col--overall">
                        <div class="tc-manage-quiz-ulq-col-label"><%: T("Overall Approved","Diluluskan Semua") %> <span class="tc-manage-quiz-info-icon" tabindex="0" data-tip="<%: T("Total approved questions available for this quiz from all teachers.","Jumlah soalan yang diluluskan tersedia untuk kuiz ini daripada semua guru.") %>"><i class="bi bi-info-circle"></i></span></div>
                        <div class="tc-manage-quiz-ulq-col-val tc-manage-quiz-val-overall"><%# Eval("overallApproved") %></div>
                    </div>
                    <div class="tc-manage-quiz-ulq-col tc-manage-quiz-ulq-col--submitted">
                        <div class="tc-manage-quiz-ulq-col-label"><%: T("Your Submitted","Hantar Anda") %> <span class="tc-manage-quiz-info-icon" tabindex="0" data-tip="<%: T("Total questions you have submitted for this quiz, including approved, pending and rejected questions.","Jumlah soalan yang telah anda hantar untuk kuiz ini, termasuk yang diluluskan, menunggu dan ditolak.") %>"><i class="bi bi-info-circle"></i></span></div>
                        <div class="tc-manage-quiz-ulq-col-val"><%# Eval("yourCount") %></div>
                    </div>
                    <div class="tc-manage-quiz-ulq-col tc-manage-quiz-ulq-col--approved">
                        ...
                        <div class="tc-manage-quiz-ulq-col-val tc-manage-quiz-val-approved"><%# Eval("approvedCount") %></div>
                    </div>
                    <div class="tc-manage-quiz-ulq-col tc-manage-quiz-ulq-col--pending">
                        ...
                        <div class="tc-manage-quiz-ulq-col-val tc-manage-quiz-val-pending"><%# Eval("pendingCount") %></div>
                    </div>
                    <div class="tc-manage-quiz-ulq-col tc-manage-quiz-ulq-col--rejected">
                        ...
                        <div class="tc-manage-quiz-ulq-col-val tc-manage-quiz-val-rejected"><%# Eval("rejectedCount") %></div>
                    </div>
                </div>
                <div class="tc-manage-quiz-ulq-btn-col">
                    <a href="#" class="tc-manage-quiz-ulq-btn tc-manage-quiz-ulq-btn-add" onclick='openSubtopicModal("<%# Eval("quizId") %>");return false;'><i class="bi bi-plus-lg"></i> <%: T("Add Questions","Tambah Soalan") %></a>
                    <button type="button" class="tc-manage-quiz-ulq-btn" onclick='openULModal("<%# Eval("quizId") %>")'><i class="bi bi-eye"></i> <%: T("View Questions","Lihat Soalan") %></button>
                </div>
            </div>
        </ItemTemplate>
    </asp:Repeater>
...
<div id="ulModal" class="tc-manage-quiz-modal-overlay" style="display:none;" onclick="if(event.target===this)closeULModal()">
    <div class="tc-manage-quiz-modal" style="max-width:880px;max-height:88vh;display:flex;flex-direction:column;">
        <div class="tc-manage-quiz-modal-header" style="background:linear-gradient(135deg,#EFF6FF,#DBEAFE);border-bottom:1px solid #BFDBFE;">
            <h3 style="color:#1E40AF;"><i class="bi bi-journal-text" style="color:#2563EB;"></i> <%: T("Submitted Questions","Soalan Dihantar") %></h3>
            <button type="button" class="tc-manage-quiz-modal-close" onclick="closeULModal()">×</button>
        </div>
        <div class="tc-manage-quiz-modal-body" id="ulModalBody" style="text-align:left;overflow-y:auto;padding:1.25rem 1.5rem;">
...
<div id="vqImgModal" class="tc-view-quiz-img-modal" onclick="if(event.target===this)closeImgPreview()">
    <div class="tc-view-quiz-img-modal-box">
        <div class="tc-view-quiz-img-modal-hd"><span id="vqImgName"></span><button type="button" class="tc-view-quiz-img-modal-close" onclick="closeImgPreview()">×</button></div>
        <div class="tc-view-quiz-img-modal-body"><img id="vqImgPreview" src="" alt="Preview" /></div>
    </div>
</div>

<%-- Subtopic Selection Modal --%>
<div id="subtopicModal" class="tc-manage-quiz-modal-overlay" style="display:none;" onclick="if(event.target===this)closeSubtopicModal()">
    <div class="tc-manage-quiz-modal" style="max-width:460px;">
        <div class="tc-manage-quiz-modal-header">
            ...
            <button type="button" class="tc-manage-quiz-modal-close" onclick="closeSubtopicModal()">×</button>
        </div>
        ...
        <div class="tc-manage-quiz-modal-footer">
            <button type="button" class="tc-manage-quiz-btn-cancel" onclick="closeSubtopicModal()"><%: T("Cancel","Batal") %></button>
            <button type="button" id="stContinueBtn" class="tc-manage-quiz-btn-create" style="box-shadow:none;opacity:.5;pointer-events:none;" onclick="goToCreatePage()"><%: T("Continue","Teruskan") %></button>
        </div>
    </div>
</div>
...
<asp:Panel ID="pnlDiscover" runat="server" Visible="false">
    <div class="tc-manage-quiz-carousel-wrap">
        <button type="button" class="tc-manage-quiz-arrow tc-manage-quiz-arrow-left" onclick="scrollDiscover(-1)"><i class="bi bi-chevron-left"></i></button>
                    ...
                    <div class="tc-manage-quiz-dcard">
                        ...
                        <div class="tc-manage-quiz-dcard-body">
                            ...
                            <div class="tc-manage-quiz-dcard-teacher">
                                <div class="tc-manage-quiz-dcard-teacher-avatar"><i class="bi bi-person-fill"></i></div>
                                <span class="tc-manage-quiz-dcard-teacher-name"><%# HttpUtility.HtmlEncode(Eval("teacherName")) %></span>
                            </div>
                        </div>
                        <div style="padding:.6rem 1.2rem .8rem;">
                            ...
                            <a href="#" class="tc-practice-quiz-btn-view" onclick="openDiscoverModal(this.previousElementSibling);return false;"><i class="bi bi-eye"></i> View Quiz</a>
                        </div>
                    </div>
                </ItemTemplate>
            </asp:Repeater>
        </div>
        <button type="button" class="tc-manage-quiz-arrow tc-manage-quiz-arrow-right" onclick="scrollDiscover(1)"><i class="bi bi-chevron-right"></i></button>
    </div>
</asp:Panel>
...
<asp:Panel ID="pnlDiscoverEmpty" runat="server" Visible="false">
    <div class="tc-manage-quiz-empty">
        ...
        <div class="tc-manage-quiz-empty-title"><%: T("No shared quizzes available.","Tiada kuiz kongsi tersedia.") %></div>
    </div>
</asp:Panel>
...
<div id="pqViewModal" class="tc-practice-quiz-overlay" onclick="if(event.target===this)closePracticeViewModal()">
    <div class="tc-practice-quiz-modal">
        <div class="tc-practice-quiz-modal-header">
            ...
            <button type="button" class="tc-practice-quiz-close-btn" onclick="closePracticeViewModal()" title="Close">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>
        <div class="tc-practice-quiz-modal-body" id="pqViewBody">
            <div class="tc-manage-quiz-empty"><div style="font-size:2rem;opacity:.4;">?</div><div class="tc-manage-quiz-empty-title" style="margin-top:.5rem;"><%: T("Loading...","Memuatkan...") %></div></div>
        </div>
    </div>
</div>
<%-- --- DISCOVER QUIZ VIEW MODAL --- --%>
<div id="dqModal" class="tc-delete-quiz-overlay" onclick="if(event.target===this)closeDiscoverModal()">
    <div class="tc-delete-quiz-modal">
        <div class="tc-delete-quiz-modal-header" id="dqHeader">
            ...
            <button type="button" class="tc-delete-quiz-close-btn" onclick="closeDiscoverModal()" title="Close">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>
...
<div id="practiceModal" class="tc-manage-quiz-modal-overlay" style="display:none;" onclick="if(event.target===this)closePracticeModal()">
    <div class="tc-manage-quiz-modal" style="max-width:460px;">
        <div class="tc-manage-quiz-modal-header">
            <div>
            ...
            <button type="button" class="tc-manage-quiz-modal-close" onclick="closePracticeModal()">×</button>
        </div>
        <div class="tc-manage-quiz-modal-body" style="text-align:left;padding:1.25rem 1.5rem;">
            <div style="margin-bottom:1rem;">
                <label style="font-size:.8rem;font-weight:700;color:var(--tc-text);display:block;margin-bottom:5px;"><%: T("Level","Tahap") %> *</label>
                <select id="pqLevel" class="tc-manage-quiz-select" style="width:100%;height:42px;" onchange="pqOnLevelChange(this.value)">
            ...
            <div style="margin-bottom:1rem;">
                ...
                <select id="pqUnit" class="tc-manage-quiz-select" style="width:100%;height:42px;" onchange="pqOnUnitChange(this.value)" disabled>
                    <option value=""><%: T("— Select Unit —","— Pilih Unit —") %></option>
                </select>
            </div>
            <div style="margin-bottom:1rem;">
                <label style="font-size:.8rem;font-weight:700;color:var(--tc-text);display:block;margin-bottom:5px;"><%: T("Subtopic","Subtopik") %> *</label>
                <select id="pqSubtopic" class="tc-manage-quiz-select" style="width:100%;height:42px;" onchange="pqOnSubtopicChange(this.value)" disabled>
                    <option value=""><%: T("— Select Subtopic —","— Pilih Subtopik —") %></option>
                </select>
            </div>
            <div style="margin-bottom:.25rem;">
                ...
                <select id="pqLanguage" class="tc-manage-quiz-select" style="width:100%;height:42px;" onchange="pqUpdateContinue()">
        ...
        <div class="tc-manage-quiz-modal-footer">
            <button type="button" class="tc-manage-quiz-btn-cancel" onclick="closePracticeModal()"><%: T("Cancel","Batal") %></button>
            <button type="button" id="pqContinueBtn" class="tc-manage-quiz-btn-create" style="box-shadow:none;opacity:.5;pointer-events:none;" onclick="pqContinue()"><%: T("Continue","Teruskan") %></button>
        </div>
    </div>
</div>
...
<div id="createModal" class="tc-manage-quiz-modal-overlay" style="display:none;">
    <div class="tc-manage-quiz-modal" style="max-width:520px;">
        <div class="tc-manage-quiz-modal-header">
            ...
            <button type="button" class="tc-manage-quiz-modal-close" onclick="document.getElementById('createModal').style.display='none'">×</button>
        </div>
        <div class="tc-manage-quiz-modal-body" style="text-align:left;padding:1.25rem 1.5rem;">
            <div style="margin-bottom:1rem;">
                ...
                <asp:DropDownList ID="ddlCreateType" runat="server" CssClass="tc-manage-quiz-select" style="width:100%;" AutoPostBack="true" OnSelectedIndexChanged="ddlCreateType_Changed" />
            ...
            <asp:Panel ID="pnlCreateLevel" runat="server" Visible="false">
                <div style="margin-bottom:1rem;">
                    ...
                    <asp:DropDownList ID="ddlCreateLevel" runat="server" CssClass="tc-manage-quiz-select" style="width:100%;" AutoPostBack="true" OnSelectedIndexChanged="ddlCreateLevel_Changed" />
            ...
            <asp:Panel ID="pnlCreateUnit" runat="server" Visible="false">
                <div style="margin-bottom:1rem;">
                    ...
                    <asp:DropDownList ID="ddlCreateUnit" runat="server" CssClass="tc-manage-quiz-select" style="width:100%;" AutoPostBack="true" OnSelectedIndexChanged="ddlCreateUnit_Changed" />
            ...
            <asp:Panel ID="pnlCreateSubtopic" runat="server" Visible="false">
                <div style="margin-bottom:1rem;">
                    ...
                    <asp:DropDownList ID="ddlCreateSubtopic" runat="server" CssClass="tc-manage-quiz-select" style="width:100%;" />
            ...
            <asp:Panel ID="pnlCreateLang" runat="server" Visible="false">
                <div style="margin-bottom:1rem;">
                    ...
                    <asp:DropDownList ID="ddlCreateLang" runat="server" CssClass="tc-manage-quiz-select" style="width:100%;">
        ...
        <div class="tc-manage-quiz-modal-footer">
            <button type="button" class="tc-manage-quiz-btn-cancel" onclick="document.getElementById('createModal').style.display='none'"><%: T("Cancel","Batal") %></button>
            <asp:Button ID="btnContinue" runat="server" CssClass="tc-manage-quiz-btn-create" style="box-shadow:none;" Text="Continue" OnClick="btnContinue_Click" CausesValidation="false" />
        </div>
    </div>
</div>
...
<div id="deleteModal" class="tc-manage-quiz-modal-overlay" style="display:none;">
    <div class="tc-manage-quiz-modal">
        <div class="tc-manage-quiz-modal-header"><h3><%: T("Delete Quiz","Padam Kuiz") %></h3><button type="button" class="tc-manage-quiz-modal-close" onclick="closeDeleteModal()">×</button></div>
        <div class="tc-manage-quiz-modal-body"><p><%: T("Are you sure you want to delete this quiz? This action cannot be undone.","Adakah anda pasti mahu memadam kuiz ini? Tindakan ini tidak boleh dibatalkan.") %></p></div>
        <div class="tc-manage-quiz-modal-footer">
            <button type="button" class="tc-manage-quiz-btn-cancel" onclick="closeDeleteModal()"><%: T("Cancel","Batal") %></button>
            <asp:Button ID="btnConfirmDelete" runat="server" CssClass="tc-manage-quiz-btn-danger" OnClick="btnConfirmDelete_Click" CausesValidation="false" />
        </div>
    </div>
</div>

<%-- Resubmit Confirmation Modal --%>
<div id="resubmitModal" class="tc-manage-quiz-modal-overlay" style="display:none;" onclick="if(event.target===this)closeResubmitModal()">
    <div class="tc-manage-quiz-modal" style="max-width:440px;">
        <div class="tc-resubmit-modal-header">
            <div class="tc-resubmit-modal-icon-wrap"><i class="bi bi-arrow-repeat"></i></div>
            <div class="tc-resubmit-modal-header-text">
                <h3 class="tc-resubmit-modal-title"><%: T("Resubmit Quiz","Hantar Semula Kuiz") %></h3>
            </div>
            <button type="button" class="tc-manage-quiz-modal-close" onclick="closeResubmitModal()">×</button>
        </div>
        <div class="tc-resubmit-modal-body">
            <p class="tc-resubmit-modal-msg"><%: T("Are you sure you want to resubmit this quiz for review?","Adakah anda pasti mahu menghantar semula kuiz ini untuk semakan?") %></p>
            <p class="tc-resubmit-modal-sub"><%: T("The quiz and all questions inside it will be changed to Pending.","Kuiz dan semua soalan di dalamnya akan ditukar kepada Menunggu.") %></p>
        </div>
        <div class="tc-resubmit-modal-footer">
            <button type="button" class="tc-resubmit-modal-btn-cancel" onclick="closeResubmitModal()"><%: T("Cancel","Batal") %></button>
            <button type="button" class="tc-resubmit-modal-btn-confirm" onclick="confirmResubmit()"><i class="bi bi-arrow-repeat"></i> <%: T("Confirm Resubmit","Sahkan Hantar Semula") %></button>
        </div>
    </div>
</div>
