<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="liveSession.aspx.cs"
    Inherits="ScienceBuddy.Teacher.liveSession" MasterPageFile="~/Site.Master"
    Title="Live Sessions" EnableEventValidation="false" %>
...
<asp:Content ID="cSidebar" ContentPlaceHolderID="SidebarMenu" runat="server">
    ...
    <div class="sb-nav-section">
        ...
        <a href="<%: ResolveUrl("~/Teacher/liveSession.aspx") %>" class="sb-sidebar-item active">
...
<asp:Content ID="cPageTitle" ContentPlaceHolderID="PageTitle" runat="server">
    <%: T("Live Sessions","Kelas Langsung") %>
</asp:Content>
...
<asp:Content ID="cMain" ContentPlaceHolderID="MainContentSidebar" runat="server">
    ...
    <div class="tc-live-session-page-header">
        <div>
            <h1><%: T("Live Sessions","Kelas Langsung") %></h1>
            <p><%: T("Manage all your online classes, schedules, and previous sessions.","Urus semua kelas dalam talian, jadual, dan sesi terdahulu anda.") %></p>
        </div>
    </div>

    <%-- Pending License Notice --%>
    <div id="lsPendingNotice" class="tc-live-session-pending-notice" style="display:none;">
        <div class="tc-live-session-pending-notice-icon">
            <i class="bi bi-shield-exclamation"></i>
        </div>
        <div class="tc-live-session-pending-notice-content">
            <div class="tc-live-session-pending-notice-title">
    ...
    <div class="tc-live-session-action-row">
        <div class="tc-live-session-action-card tc-live-session-action-schedule"
            onclick="document.getElementById('scheduleModal').style.display='flex'">
            <div class="tc-live-session-action-icon">
                <i class="bi bi-calendar-plus"></i>
            </div>
            <div>
                <div class="tc-live-session-action-title">
                    <%: T("Schedule Live Class","Jadualkan Kelas Langsung") %>
                </div>
                <div class="tc-live-session-action-desc">
                    <%: T("Plan a future online class","Rancang kelas dalam talian") %>
                </div>
            </div>
            <div class="tc-live-session-action-cta">
                <%: T("Schedule","Jadual") %> <i class="bi bi-arrow-right"></i>
            </div>
        </div>
        <div class="tc-live-session-action-card tc-live-session-action-instant"
            onclick="document.getElementById('instantModal').style.display='flex'">
            <div class="tc-live-session-action-icon">
                <i class="bi bi-broadcast-pin"></i>
                <span class="tc-live-session-live-dot"></span>
            </div>
            <div>
                <div class="tc-live-session-action-title">
                    <%: T("Start Instant Class","Mulakan Kelas Segera") %>
                </div>
                <div class="tc-live-session-action-desc">
                    <%: T("Begin teaching immediately","Mula mengajar serta-merta") %>
                </div>
            </div>
            <div class="tc-live-session-action-cta">
                <%: T("Start Now","Mula Sekarang") %> <i class="bi bi-arrow-right"></i>
            </div>
        </div>
    </div>
    ...
    <div class="tc-live-session-stats">
        <div class="tc-live-session-stat tc-live-session-stat-upcoming">
            <div class="tc-live-session-stat-icon" style="background:#DBEAFE;color:#2563EB;">
                <i class="bi bi-calendar-event"></i>
            </div>
            <div class="tc-live-session-stat-val">
                <asp:Literal ID="litUpcoming" runat="server" Text="0" />
            </div>
            <div class="tc-live-session-stat-label">
                <%: T("Upcoming Sessions","Sesi Akan Datang") %>
            </div>
        </div>
        <div class="tc-live-session-stat tc-live-session-stat-completed">
            <div class="tc-live-session-stat-icon" style="background:#D1FAE5;color:#047857;">
            ...
            <div class="tc-live-session-stat-label">
                <%: T("Completed Sessions","Sesi Selesai") %>
            </div>
        </div>
        <div class="tc-live-session-stat tc-live-session-stat-cancelled">
            <div class="tc-live-session-stat-icon" style="background:#FEE2E2;color:#B91C1C;">
            ...
            <div class="tc-live-session-stat-label">
                <%: T("Cancelled Sessions","Sesi Dibatalkan") %>
            </div>
        </div>
        <div class="tc-live-session-stat tc-live-session-stat-students">
            <div class="tc-live-session-stat-icon" style="background:#E0E7FF;color:#4338CA;">
                <i class="bi bi-people"></i>
            </div>
            <div class="tc-live-session-stat-val">
                <asp:Literal ID="litStudentsJoined" runat="server" Text="0" />
            </div>
            <div class="tc-live-session-stat-label">
                <%: T("Students Joined","Pelajar Menyertai") %>
            </div>
        </div>
    </div>
    ...
    <div class="tc-live-session-tabs">
        <button type="button" class="tc-live-session-tab active" id="btnTabUpcoming"
            onclick="switchLsTab('upcoming')">
            <%: T("Upcoming","Akan Datang") %>
        </button>
        <button type="button" class="tc-live-session-tab" id="btnTabHistory"
            onclick="switchLsTab('history')">
            <%: T("History","Sejarah") %>
        </button>
    </div>
    ...
    <div id="lsTabUpcoming">
                ...
                <asp:Repeater ID="rptUpcoming" runat="server" OnItemCommand="rptSessions_ItemCommand">
                    <ItemTemplate>
                        <div class="tc-live-session-card">
                            <div class="tc-live-session-card-date">
                                <span class="tc-live-session-card-day"><%# Eval("day") %></span>
                                <span class="tc-live-session-card-month"><%# Eval("month") %></span>
                            </div>
                            <div class="tc-live-session-card-body">
                                <div class="tc-live-session-card-title">
                                ...
                                <span class='tc-live-session-badge <%# Eval("badgeCss") %>'>
                                    <%# Eval("badgeLabel") %>
                                </span>
                            </div>
                            <div class="tc-live-session-card-actions">
                                <asp:LinkButton ID="btnStartSession" runat="server"
                                    CommandName="StartSession"
                                    CommandArgument='<%# Eval("sessionId") %>'
                                    CssClass='<%# (bool)Eval("canStart") ? "tc-live-session-act tc-live-session-act-start" : "tc-live-session-act tc-live-session-act-notyet" %>'
                                    CausesValidation="false"
                                    Enabled='<%# (bool)Eval("canStart") %>'>
                                ...
                                <asp:LinkButton ID="btnReschedule" runat="server"
                                    ...
                                    CommandArgument='<%# Eval("sessionId") + "|" + Eval("rawStart") + "|" + Eval("rawEnd") + "|" + HttpUtility.HtmlEncode(Eval("title")) %>'
                                    CssClass="tc-live-session-act tc-live-session-act-edit"
                                    CausesValidation="false"
                                    Visible='<%# !(bool)Eval("canStart") %>'>
                                    <i class="bi bi-calendar2-plus"></i> <%: T("Reschedule","Jadual Semula") %>
                                </asp:LinkButton>
                                <asp:LinkButton ID="btnDel" runat="server"
                                    CommandName="Cancel"
                                    CommandArgument='<%# Eval("sessionId") + "|" + HttpUtility.HtmlEncode(Eval("title")) + "|" + Eval("timeRange") %>'
                                    CssClass="tc-live-session-act tc-live-session-act-cancel"
                                    CausesValidation="false">
        ...
        <asp:Panel ID="pnlUpcomingEmpty" runat="server" Visible="false">
            <div class="tc-live-session-empty">
                ...
                <div class="tc-live-session-empty-title">
                    <%: T("No Upcoming Sessions","Tiada Sesi Akan Datang") %>
                </div>
                <div class="tc-live-session-empty-sub">
                    <%: T("You do not have any upcoming live classes scheduled.","Anda tidak mempunyai kelas langsung yang dijadualkan.") %>
                </div>
            </div>
        </asp:Panel>
    </div>
    ...
    <div id="lsTabHistory" style="display:none;">
        ...
        <asp:Panel ID="pnlHistoryEmpty" runat="server" Visible="false">
            <div class="tc-live-session-empty">
                ...
                <div class="tc-live-session-empty-title">
                    <%: T("No Session History","Tiada Sejarah Sesi") %>
                </div>
                <div class="tc-live-session-empty-sub">
                    <%: T("Your completed and cancelled live classes will appear here.","Kelas langsung yang selesai dan dibatalkan akan terpapar di sini.") %>
                </div>
            </div>
        </asp:Panel>
    </div>
    ...
    <div id="instantModal" class="tc-live-session-modal-overlay" style="display:none;">
        <div class="tc-live-session-modal">
            <div class="tc-live-session-modal-header">
                <h3 style="font-size:1.1rem;">
                    <%: T("Start Instant Class","Mulakan Kelas Segera") %>
                </h3>
                <button type="button" class="tc-live-session-modal-close"
                    onclick="document.getElementById('instantModal').style.display='none'">
                    &times;
                </button>
            </div>
            <div class="tc-live-session-modal-body">
                <p style="font-size:.82rem;color:var(--tm);margin:0 0 1rem;">
                    <%: T("Create a live class and begin teaching immediately.","Cipta kelas langsung dan mula mengajar serta-merta.") %>
                </p>
                <div class="tc-live-session-field">
                    <label class="tc-live-session-label">
                        <%: T("Session Title","Tajuk Sesi") %> <span style="color:var(--te);">*</span>
                    </label>
                    <asp:TextBox ID="txtInstantTitle" runat="server"
                        CssClass="tc-live-session-input" MaxLength="200" />
                    <div class="tc-live-session-field-error" id="errInstTitle">
                        Please enter a session title.
                    </div>
                </div>
                <div class="tc-live-session-field">
                    <label class="tc-live-session-label">
                        <%: T("Description","Penerangan") %>
                    </label>
                    <asp:TextBox ID="txtInstantDesc" runat="server"
                        CssClass="tc-live-session-input" TextMode="MultiLine" Rows="2" />
                </div>
                <div class="tc-live-session-row2">
                    <div class="tc-live-session-field">
                        <label class="tc-live-session-label">
                            <%: T("Unit","Unit") %> <span style="color:var(--te);">*</span>
                        </label>
                        <asp:DropDownList ID="ddlInstantUnit" runat="server"
                            CssClass="tc-live-session-input" />
                        <div class="tc-live-session-field-error" id="errInstUnit">
                            Please select a unit.
                        </div>
                    </div>
                    <div class="tc-live-session-field">
                        <label class="tc-live-session-label">
                            <%: T("Subtopic","Subtopik") %> <span style="color:var(--te);">*</span>
                        </label>
                        <asp:DropDownList ID="ddlInstantSubtopic" runat="server"
                            CssClass="tc-live-session-input" />
                        <div class="tc-live-session-field-error" id="errInstSub">
                            Please select a subtopic.
                        </div>
                    </div>
                </div>
                <asp:Panel ID="pnlInstantError" runat="server" Visible="false">
                    <div style="font-size:.78rem;color:var(--te);font-weight:600;margin-top:.25rem;">
                        <asp:Literal ID="litInstantError" runat="server" />
                    </div>
                </asp:Panel>
            </div>
            <div class="tc-live-session-modal-footer">
                <button type="button" class="tc-live-session-btn-cancel-modal"
                    onclick="document.getElementById('instantModal').style.display='none'">
                    <%: T("Cancel","Batal") %>
                </button>
                <asp:Button ID="btnStartLive" runat="server"
                    CssClass="tc-live-session-btn-submit" style="background:#DC2626;"
                    OnClick="btnStartLive_Click" CausesValidation="false"
                    OnClientClick="return validateInstantForm();" />
            </div>
        </div>
    </div>
    <asp:HiddenField ID="hidShowInstantModal" runat="server" Value=""
        OnValueChanged="hidShowInstantModal_ValueChanged" />

    <%-- Reschedule Modal --%>
    <div id="rescheduleModal" class="tc-live-session-modal-overlay" style="display:none;">
        <div class="tc-live-session-modal">
            <div class="tc-live-session-modal-header">
                <h3><%: T("Reschedule Live Session","Jadual Semula Sesi Langsung") %></h3>
                <button type="button" class="tc-live-session-modal-close"
            ...
            <div class="tc-live-session-modal-body">
                <div style="background:#F9FAFB;border-radius:10px;padding:.75rem 1rem;margin-bottom:1rem;font-size:.82rem;color:var(--tt);">
                    ...
                    <span style="font-size:.76rem;color:var(--tm);">
                        <asp:Literal ID="litRescCurrent" runat="server" />
                    </span>
                </div>
                <asp:HiddenField ID="hidRescId" runat="server" Value="" />
                <div class="tc-live-session-field">
                    <label class="tc-live-session-label">
                        <%: T("New Date","Tarikh Baharu") %> *
                    </label>
                    <asp:TextBox ID="txtRescDate" runat="server"
                        CssClass="tc-live-session-input" TextMode="Date" />
                </div>
                <div class="tc-live-session-row2">
                    <div class="tc-live-session-field">
                        <label class="tc-live-session-label">
                            <%: T("New Start Time","Masa Mula Baharu") %> *
                        </label>
                        <asp:TextBox ID="txtRescStart" runat="server"
                            CssClass="tc-live-session-input" TextMode="Time" />
                    </div>
                    <div class="tc-live-session-field">
                        <label class="tc-live-session-label">
                            <%: T("New End Time","Masa Tamat Baharu") %> *
                        </label>
                        <asp:TextBox ID="txtRescEnd" runat="server"
                            CssClass="tc-live-session-input" TextMode="Time" />
                    </div>
                </div>
                <asp:Panel ID="pnlRescError" runat="server" Visible="false">
                    <div style="font-size:.76rem;color:var(--te);font-weight:600;">
                        <asp:Literal ID="litRescError" runat="server" />
                    </div>
                </asp:Panel>
            </div>
            <div class="tc-live-session-modal-footer">
                ...
                <asp:Button ID="btnSaveReschedule" runat="server"
                    CssClass="tc-live-session-btn-submit"
                    OnClick="btnSaveReschedule_Click" CausesValidation="false" />
            </div>
        </div>
    </div>
    ...
    <div id="cancelModal" class="tc-live-session-modal-overlay" style="display:none;">
        <div class="tc-live-session-modal">
            <div class="tc-live-session-modal-header">
                <h3><%: T("Cancel Live Session?","Batalkan Sesi Langsung?") %></h3>
                <button type="button" class="tc-live-session-modal-close"
            ...
            <div class="tc-live-session-modal-body" style="text-align:center;padding:1.5rem;">
                <p style="font-size:.88rem;color:var(--tt);margin:0 0 .5rem;">
                    <%: T("Are you sure you want to cancel this live session?","Adakah anda pasti mahu membatalkan sesi langsung ini?") %>
                </p>
                <div style="background:#FEF2F2;border-radius:10px;padding:.75rem 1rem;font-size:.82rem;color:#B91C1C;margin-top:.75rem;">
                    ...
                    <span style="font-size:.76rem;">
                        <asp:Literal ID="litCancelTime" runat="server" />
                    </span>
                </div>
            ...
            <div class="tc-live-session-modal-footer" style="justify-content:center;">
                <button type="button" class="tc-live-session-btn-cancel-modal"
                    onclick="document.getElementById('cancelModal').style.display='none'">
                    <%: T("Keep Session","Kekalkan Sesi") %>
                </button>
                <asp:Button ID="btnConfirmCancel" runat="server"
                    CssClass="tc-live-session-btn-submit" style="background:var(--te);"
                    OnClick="btnConfirmCancel_Click" CausesValidation="false" />
            </div>
        </div>
    </div>
    ...
    <div id="scheduleModal" class="tc-live-session-modal-overlay" style="display:none;">
        <div class="tc-live-session-modal">
            <div class="tc-live-session-modal-header">
                <h3><%: T("Schedule Live Class","Jadualkan Kelas Langsung") %></h3>
                <button type="button" class="tc-live-session-modal-close"
                    onclick="document.getElementById('scheduleModal').style.display='none'">
                    &times;
                </button>
            </div>
            <div class="tc-live-session-modal-body">
                <div class="tc-live-session-field">
                    <label class="tc-live-session-label">
                        <%: T("Class Title","Tajuk Kelas") %> <span style="color:var(--te);">*</span>
                    </label>
                    <asp:TextBox ID="txtTitle" runat="server"
                        CssClass="tc-live-session-input" MaxLength="200" />
                    <div class="tc-live-session-field-error" id="errSchTitle">
                        Please enter a class title.
                    </div>
                </div>
                <div class="tc-live-session-field">
                    <label class="tc-live-session-label">
                        <%: T("Date","Tarikh") %> <span style="color:var(--te);">*</span>
                    </label>
                    <asp:TextBox ID="txtDate" runat="server"
                        CssClass="tc-live-session-input" TextMode="Date" />
                    <div class="tc-live-session-field-error" id="errSchDate">
                ...
                <div class="tc-live-session-row2">
                    <div class="tc-live-session-field">
                        <label class="tc-live-session-label">
                            <%: T("Start Time","Masa Mula") %> <span style="color:var(--te);">*</span>
                        </label>
                        <asp:TextBox ID="txtStart" runat="server"
                            CssClass="tc-live-session-input" TextMode="Time" />
                        <div class="tc-live-session-field-error" id="errSchStart">
                            Please select a start time.
                        </div>
                    </div>
                    <div class="tc-live-session-field">
                        <label class="tc-live-session-label">
                            <%: T("End Time","Masa Tamat") %> <span style="color:var(--te);">*</span>
                        </label>
                        <asp:TextBox ID="txtEnd" runat="server"
                            CssClass="tc-live-session-input" TextMode="Time" />
                        <div class="tc-live-session-field-error" id="errSchEnd">
                            Please select an end time.
                        </div>
                    </div>
                </div>
                <div class="tc-live-session-field-error" id="errSchDuration"
                    style="margin-top:-0.5rem;margin-bottom:.75rem;">
                </div>
                <div class="tc-live-session-field">
                    <label class="tc-live-session-label">
                        <%: T("Unit","Unit") %> <span style="color:var(--te);">*</span>
                    ...
                    <asp:DropDownList ID="ddlUnit" runat="server"
                        CssClass="tc-live-session-input" />
                    <div class="tc-live-session-field-error" id="errSchUnit">
                        Please select a unit.
                    </div>
                </div>
                <div class="tc-live-session-field">
                    ...
                    <asp:DropDownList ID="ddlSubtopic" runat="server"
                        CssClass="tc-live-session-input" />
                    <div class="tc-live-session-field-error" id="errSchSub">
                        Please select a subtopic.
                    </div>
                </div>
                <div class="tc-live-session-field">
            ...
            <div class="tc-live-session-modal-footer">
                ...
                <asp:Button ID="btnSchedule" runat="server"
                    CssClass="tc-live-session-btn-submit"
                    OnClick="btnSchedule_Click" CausesValidation="false"
    ...
    <asp:Panel ID="pnlSummary" runat="server">
        ...
        <div class="tc-live-session-summary-panel" id="lsSummaryPanel" style="display:none;">
            <div class="tc-live-session-summary-hdr">
                <div class="tc-live-session-summary-ico">
                    <i class="bi bi-check-lg"></i>
                </div>
                <div class="tc-live-session-summary-hdr-text">
                    <h3><%: T("Live Session Completed","Sesi Langsung Selesai") %></h3>
                    <div class="tc-live-session-summary-hdr-sub">
                        <asp:Literal ID="litSumTitle" runat="server" />
                    </div>
                </div>
            </div>
            <div class="tc-live-session-summary-body">
                <div class="tc-live-session-sum-times">
                    <div class="tc-live-session-sum-time">
                        <%: T("Start","Mula") %>
                        <span><asp:Literal ID="litSumStart" runat="server" /></span>
                    </div>
                    <div class="tc-live-session-sum-time">
                        <%: T("End","Tamat") %>
                        <span><asp:Literal ID="litSumEnd" runat="server" /></span>
                    </div>
                </div>
                <div class="tc-live-session-sum-stats">
                    <div class="tc-live-session-sum-stat">
                        <div class="tc-live-session-sum-stat-ico" style="background:#EDE9FE;color:#7C3AED;">
                            <i class="bi bi-clock-fill"></i>
                        </div>
                        <div>
                            <div class="tc-live-session-sum-stat-val">
                                <asp:Literal ID="litSumDuration" runat="server" />
                            </div>
                            <div class="tc-live-session-sum-stat-lbl">
                                <%: T("Duration","Tempoh") %>
                            </div>
                        </div>
                    </div>
                    <div class="tc-live-session-sum-stat">
                        <div class="tc-live-session-sum-stat-ico" style="background:#D1FAE5;color:#059669;">
                            <i class="bi bi-people-fill"></i>
                        </div>
                        <div>
                            <div class="tc-live-session-sum-stat-val">
                                <asp:Literal ID="litSumStudents" runat="server" />
                            </div>
                            <div class="tc-live-session-sum-stat-lbl">
                                <%: T("Students Joined","Pelajar Menyertai") %>
                            </div>
                ...
                <div class="tc-live-session-sum-parts">
                    <div class="tc-live-session-sum-parts-title">
                        <i class="bi bi-person-lines-fill" style="color:var(--tp);"></i>
                        <%: T("Participants","Peserta") %>
                    </div>
                            ...
                            <asp:Repeater ID="rptSumParticipants" runat="server">
                                <ItemTemplate>
                                    <div class="tc-live-session-sum-part-item">
                                        <div class="tc-live-session-sum-part-av">
                                            <%# Eval("initials") %>
                                        </div>
                                        <div class="tc-live-session-sum-part-name">
                    ...
                    <asp:Panel ID="pnlSumNoParticipants" runat="server" Visible="false">
                        <div class="tc-live-session-sum-no-parts">
                            <%: T("No students attended this live session.","Tiada pelajar menghadiri sesi langsung ini.") %>
                        </div>
                    </asp:Panel>
                    <div class="tc-live-session-sum-no-parts" id="lsSumNoPartsJs" style="display:none;">
                        <%: T("No students attended this live session.","Tiada pelajar menghadiri sesi langsung ini.") %>
                    </div>
                </div>
            </div>
            <div class="tc-live-session-summary-ftr">
                <button type="button" class="tc-live-session-sum-btn-done"
                    onclick="closeLsSummary()">
                    <%: T("Close","Tutup") %>
                </button>
            </div>
        </div>
...
<script>
    ...
    window.addEventListener('load', function () {
        ...
        if (h && h.value) {
            var w = document.getElementById('lsToast'), t = document.createElement('div');
            t.className = 'tc-live-session-toast';
            ...
            setTimeout(function () { t.remove(); }, 3e3);
        }
        ...
        if (ss && ss.value === '1') {
            var sp = document.getElementById('lsSummaryPanel');
            var so = document.getElementById('lsSummaryOverlay');
        ...
        if (lic && lic.value === 'Pending') {
            var notice = document.getElementById('lsPendingNotice');
            if (notice) notice.style.display = 'flex';
            var cards = document.querySelectorAll('.tc-live-session-action-card');
            for (var i = 0; i < cards.length; i++) {
                cards[i].classList.add('tc-live-session-disabled');
