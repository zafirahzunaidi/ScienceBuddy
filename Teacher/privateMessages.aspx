...
<div class="tc-private-messages-left">
    <div class="tc-private-messages-left-header">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:.6rem;">
            <div class="tc-private-messages-search-wrap" style="flex:1;"><i class="bi bi-search"></i>
                <asp:TextBox ID="txtSearchConv" runat="server" CssClass="tc-private-messages-search" placeholder="Search conversations..." /></div>
            <asp:LinkButton ID="btnCompose" runat="server" CssClass="tc-private-messages-compose-btn" OnClick="btnCompose_Click" CausesValidation="false" ToolTip="New Message">
    ...
    <div class="tc-private-messages-list" id="pmConvList">
        <div id="pmNoConvsMsg" class="tc-private-messages-empty" style="display:none;"><i class="bi bi-chat-square-text"></i><p style="font-weight:700;margin-bottom:.25rem;">No matching conversations found.</p><span style="font-size:.78rem;color:var(--tm);">Try searching with a different name or selecting another category.</span></div>
        <asp:Panel ID="pnlNoConvs" runat="server" Visible="false">
            <div class="tc-private-messages-empty"><i class="bi bi-chat-square-text"></i><p><asp:Literal ID="litNoConvMsg" runat="server" /></p></div>
        </asp:Panel>
        <asp:Repeater ID="rptConvs" runat="server" OnItemCommand="rptConvs_ItemCommand">
            <ItemTemplate>
                <div class='tc-private-messages-conv <%# Eval("chatId").ToString() == SelectedChatId ? "active" : "" %>' data-name='<%# HttpUtility.HtmlAttributeEncode(Eval("name").ToString().ToLower()) %>' data-role='<%# Eval("role") %>' data-chatid='<%# Eval("chatId") %>'>
                    <asp:LinkButton ID="lnkConv" runat="server" CommandName="Select" CommandArgument='<%# Eval("chatId") %>' CausesValidation="false" style="display:contents;">
                        ...
                        <div class="tc-private-messages-conv-info">
                            <div class="tc-private-messages-conv-name"><%# HttpUtility.HtmlEncode(Eval("name")) %> <span class='tc-private-messages-conv-role <%# Eval("role").ToString()=="Parent"?"parent":"" %>'><%# Eval("role") %></span></div>
...
<div class="tc-private-messages-right">
    ...
    <asp:Panel ID="pnlCompose" runat="server" Visible="false">
        <div class="tc-private-messages-chat-header" style="border-bottom:1px solid var(--tb);">
            <div style="display:flex;align-items:center;gap:8px;flex:1;">
                ...
                <asp:DropDownList ID="ddlRecipientType" runat="server" CssClass="tc-private-messages-compose-select" style="display:none;" />
                <asp:DropDownList ID="ddlRecipient" runat="server" CssClass="tc-private-messages-compose-select" style="flex:1;" />
            </div>
        </div>
        ...
        <div class="tc-private-messages-composer-bottom">
            ...
            <div class="tc-private-messages-file-preview" id="pmComposeFilePreview" style="display:none;">
                <div class="tc-private-messages-file-preview-inner">
                    ...
                    <button type="button" class="tc-private-messages-file-rm" onclick="pmComposeRemoveFile()"><i class="bi bi-x-lg"></i></button>
                </div>
            </div>
            <div class="tc-private-messages-input-area">
                <asp:FileUpload ID="fuComposeAttachment" runat="server" style="display:none;" />
                <button type="button" class="tc-private-messages-icon-btn" onclick="document.getElementById('<%=fuComposeAttachment.ClientID%>').click();" title="Attach file"><i class="bi bi-plus-lg"></i></button>
                <button type="button" class="tc-private-messages-icon-btn" id="pmComposeEmojiBtn" onclick="pmComposeToggleEmoji()" title="Emoji"><i class="bi bi-emoji-smile"></i></button>
                <asp:TextBox ID="txtComposeMsg" runat="server" CssClass="tc-private-messages-input" TextMode="MultiLine" Rows="1" placeholder="Type a message..." />
                <asp:LinkButton ID="btnComposeSend" runat="server" CssClass="tc-private-messages-send-btn" OnClick="btnComposeSend_Click" CausesValidation="false">
    ...
    <asp:Panel ID="pnlChat" runat="server" Visible="false" CssClass="tc-private-messages-chat-panel">
        <div class="tc-private-messages-chat-header">
            <div class="tc-private-messages-chat-avatar"><asp:Literal ID="litChatInitials" runat="server" /></div>
            <div><div class="tc-private-messages-chat-name"><asp:Literal ID="litChatName" runat="server" /></div>
                <div class="tc-private-messages-chat-role"><asp:Literal ID="litChatRole" runat="server" /></div></div>
        </div>
            ...
            <asp:Repeater ID="rptMessages" runat="server">
                <ItemTemplate>
                    <div class='tc-private-messages-msg <%# Eval("isSent").ToString()=="True"?"sent":"received" %>'>
                        ...
                        <div class="tc-private-messages-msg-time"><%# Eval("timeStr") %><%# Convert.ToBoolean(Eval("isSent")) ? (Convert.ToBoolean(Eval("isRead")) ? "<span class='tc-private-messages-tick read'>?</span>" : "<span class='tc-private-messages-tick sent'>?</span>") : "" %></div>
                    </div>
                </ItemTemplate>
            </asp:Repeater>
            <asp:Panel ID="pnlNoMessages" runat="server" Visible="false">
                <div class="tc-private-messages-empty" style="padding:2rem;"><i class="bi bi-chat"></i><p><%: T("No messages yet. Send the first message.","Tiada mesej lagi. Hantar mesej pertama.") %></p></div>
            </asp:Panel>
        </div>
        <div class="tc-private-messages-composer-bottom">
            ...
            <div class="tc-private-messages-file-preview" id="pmFilePreview" style="display:none;">
                <div class="tc-private-messages-file-preview-inner">
                    ...
                    <button type="button" class="tc-private-messages-file-rm" onclick="pmRemoveFile()"><i class="bi bi-x-lg"></i></button>
                </div>
            </div>
            <div class="tc-private-messages-input-area">
                <asp:FileUpload ID="fuAttachment" runat="server" style="display:none;" />
                <button type="button" class="tc-private-messages-icon-btn" onclick="document.getElementById('<%=fuAttachment.ClientID%>').click();" title="Attach file"><i class="bi bi-plus-lg"></i></button>
                <button type="button" class="tc-private-messages-icon-btn" id="pmEmojiBtn" onclick="pmToggleEmoji()" title="Emoji"><i class="bi bi-emoji-smile"></i></button>
                <asp:TextBox ID="txtMessage" runat="server" CssClass="tc-private-messages-input" TextMode="MultiLine" Rows="1" placeholder="Type your message..." />
                <asp:LinkButton ID="btnSend" runat="server" CssClass="tc-private-messages-send-btn" OnClick="btnSend_Click" CausesValidation="false">
                    <i class="bi bi-send-fill"></i>
                </asp:LinkButton>
            </div>
            <%-- Emoji picker --%>
