...
<asp:Content ID="cSidebar" ContentPlaceHolderID="SidebarMenu" runat="server">
    <div class="sb-nav-section"><div class="sb-nav-section-label"><%: T("Main","Utama") %></div>
        ...
        <a href="<%: ResolveUrl("~/Teacher/Notifications.aspx") %>" class="sb-sidebar-item active"><i class="bi bi-bell item-icon"></i><span class="item-label"><%: T("Notifications","Notifikasi") %></span></a></div>
...
<div class="tc-notifications-header"><h1><%: T("Notifications","Notifikasi") %></h1><p><%: T("Stay updated with your latest ScienceBuddy activities.","Kekal dikemas kini dengan aktiviti ScienceBuddy terkini anda.") %></p></div>

<%-- Segmented Tabs with counts --%>
<div class="tc-notifications-tabs">
    <asp:LinkButton ID="btnTabAll" runat="server" CssClass="tc-notifications-tab active" OnClick="btnTab_Click" CommandArgument="All" CausesValidation="false">
        <%: T("All","Semua") %> <span class="tc-notifications-tab-count"><asp:Literal ID="litCountAll" runat="server" Text="0" /></span>
    </asp:LinkButton>
    <asp:LinkButton ID="btnTabUnread" runat="server" CssClass="tc-notifications-tab" OnClick="btnTab_Click" CommandArgument="Unread" CausesValidation="false">
        <%: T("Unread","Belum Dibaca") %> <span class="tc-notifications-tab-count"><asp:Literal ID="litCountUnread" runat="server" Text="0" /></span>
    </asp:LinkButton>
    <asp:LinkButton ID="btnTabRead" runat="server" CssClass="tc-notifications-tab" OnClick="btnTab_Click" CommandArgument="Read" CausesValidation="false">
        <%: T("Read","Dibaca") %> <span class="tc-notifications-tab-count"><asp:Literal ID="litCountRead" runat="server" Text="0" /></span>
    </asp:LinkButton>
</div>
        ...
        <asp:Repeater ID="rptNotifs" runat="server" OnItemCommand="rptNotifs_ItemCommand">
            <ItemTemplate>
                <div class='tc-notifications-card <%# Convert.ToBoolean(Eval("isRead")) ? "" : "unread" %>'>
                    <div class="tc-notifications-card-icon"><i class="bi bi-bell-fill"></i></div>
                    <div class="tc-notifications-card-body">
                        <div class="tc-notifications-card-title"><%# !Convert.ToBoolean(Eval("isRead")) ? "<span class='tc-notifications-dot'></span>" : "" %><%# HttpUtility.HtmlEncode(Eval("title")) %></div>
                        <div class="tc-notifications-card-msg"><%# HttpUtility.HtmlEncode(Eval("message")) %></div>
                        <div class="tc-notifications-card-time"><i class="bi bi-clock"></i> <%# Eval("timeDisplay") %></div>
                    </div>
                    <asp:LinkButton ID="btnMark" runat="server" CommandName="MarkRead" CommandArgument='<%# Eval("notificationId") %>'
                        CssClass="tc-notifications-mark-btn" CausesValidation="false" Visible='<%# !Convert.ToBoolean(Eval("isRead")) %>'>
...
<asp:Panel ID="pnlEmpty" runat="server" Visible="false">
    <div class="tc-notifications-empty">
        <div class="tc-notifications-empty-icon"><i class="bi bi-check-lg"></i></div>
        <div class="tc-notifications-empty-title"><%: T("You're all caught up!","Anda sudah dikemas kini!") %></div>
        <div class="tc-notifications-empty-sub"><%: T("There are no notifications yet.","Tiada pemberitahuan lagi.") %></div>
        <div class="tc-notifications-empty-caption"><%: T("We'll notify you whenever there's something new.","Kami akan memberitahu anda apabila ada sesuatu yang baharu.") %></div>
    </div>
</asp:Panel>
...
window.addEventListener('load',function(){
    ...
    if(h&&h.value){
        var w=document.getElementById('tcNotificationsToastWrap'),t=document.createElement('div');
        t.className='tc-notifications-toast';t.innerHTML='<i class="bi bi-check-circle-fill"></i> '+h.value;
