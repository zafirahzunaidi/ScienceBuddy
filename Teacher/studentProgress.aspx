...
<div class="tc-student-progress-search-card">
    <div class="tc-student-progress-search-row">
        <div class="tc-student-progress-search-wrap"><i class="bi bi-search"></i>
            <input type="text" id="spSearchInput" class="tc-student-progress-search-input" placeholder="<%: T("Search student...","Cari pelajar...") %>" value="<%= HttpUtility.HtmlAttributeEncode(txtSearch.Text) %>" /></div>
        <asp:DropDownList ID="ddlSort" runat="server" CssClass="tc-student-progress-sort" onchange="spFilterTable();return false;" />
        <button type="button" class="tc-student-progress-btn" onclick="spFilterTable()"><%: T("Search","Cari") %></button>
        <button type="button" class="tc-student-progress-btn-reset" onclick="spResetFilter()"><%: T("Reset","Set Semula") %></button>
    </div>
</div>
<asp:TextBox ID="txtSearch" runat="server" Style="display:none;" />

<div class="tc-student-progress-rank-card">
    <div class="tc-student-progress-rank-hd"><i class="bi bi-trophy"></i><h2><%: T("Student Progress Summary","Ringkasan Prestasi Pelajar") %></h2></div>

    <asp:Panel ID="pnlRankingEmpty" runat="server" Visible="false">
        <div class="tc-student-progress-empty"><i class="bi bi-people"></i>
            <div class="tc-student-progress-empty-title"><%: T("No students found.","Tiada pelajar ditemui.") %></div>
            <div class="tc-student-progress-empty-sub"><%: T("Try searching another student name.","Cuba cari nama pelajar lain.") %></div>
        </div>
    </asp:Panel>

    <asp:Panel ID="pnlTable" runat="server" Visible="false">
    <table class="tc-student-progress-tbl"><thead><tr>
    ...
    <asp:Repeater ID="rptRanking" runat="server">
        ...
        <tr>
            ...
            <td><%# Eval("unitQuizStr") %><span class='tc-student-progress-q-badge <%# Eval("unitBadge") %>'><%# BadgeLabel(Eval("unitBadge").ToString()) %></span></td>
            <td><%# Eval("levelQuizStr") %><span class='tc-student-progress-q-badge <%# Eval("levelBadge") %>'><%# BadgeLabel(Eval("levelBadge").ToString()) %></span></td>
            <td><strong><%# Eval("overallStr") %></strong></td>
            <td><a href='<%# ResolveUrl("~/Teacher/studentDetails.aspx") + "?studentId=" + Eval("studentId") %>' class="tc-student-progress-vd-btn"><i class="bi bi-eye"></i> <%: T("View Details","Lihat Butiran") %></a></td>
        </tr>
        </ItemTemplate>
    </asp:Repeater>
    </tbody></table>
