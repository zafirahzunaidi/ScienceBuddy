...
<style>
/* ── Progress & Rewards ── */
:root{--pr-orange:#FF6B2C;--pr-gold:#FFD84D;--pr-dark:#E85B1D;--pr-light:#FFF8F0;--pr-mid:#FF8C54;}

/* ══ HERO SECTION ══ */
.pr-hero{background:linear-gradient(135deg,#FF6B2C 0%,#FF8C54 40%,#FFD84D 100%);
    ...
    box-shadow:0 8px 32px rgba(255,107,44,.25);}
.pr-hero::before{content:'🏆';position:absolute;font-size:6rem;opacity:.12;
    top:-10px;right:30px;pointer-events:none;line-height:1;}
.pr-hero::after{content:'⭐';position:absolute;font-size:4rem;opacity:.10;
    bottom:10px;right:140px;pointer-events:none;line-height:1;}
...
.pr-xp-bar-inner{height:100%;border-radius:12px;
    background:linear-gradient(90deg,#FF6B2C,#FFD84D);
...
.pr-stat-value{font-family:var(--font-primary);font-size:1.75rem;font-weight:800;
    color:var(--pr-orange);line-height:1;}
...
.pr-badge-card.earned{border-color:var(--pr-gold);
    box-shadow:0 4px 16px rgba(255,216,77,.20);}
.pr-badge-card.earned::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;
    background:linear-gradient(90deg,var(--pr-orange),var(--pr-gold));}
.pr-badge-card.locked{opacity:.65;filter:grayscale(.4);}
...
.pr-badge-xp{font-size:.75rem;font-weight:700;color:var(--pr-orange);}
...
.pr-badge-status.locked-tag{background:#F3F4F6;color:#6B7280;}
.pr-lock-icon{font-size:1.5rem;color:#9CA3AF;position:absolute;top:12px;right:12px;}
...
.pr-activity-icon{width:36px;height:36px;border-radius:50%;
    background:linear-gradient(135deg,var(--pr-orange),var(--pr-gold));
...
.pr-activity-xp{font-size:.9375rem;font-weight:800;color:var(--pr-orange);}
...
.pr-cert-btn{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;
    ...
    text-decoration:none;background:var(--pr-orange);color:#fff;
    transition:all .2s;border:none;cursor:pointer;}
.pr-cert-btn:hover{background:var(--pr-dark);transform:translateY(-1px);
    box-shadow:0 4px 12px rgba(255,107,44,.30);text-decoration:none;color:#fff;}

/* ══ NAVIGATION BUTTONS ══ */
.pr-nav{display:flex;flex-wrap:wrap;gap:var(--space-md);margin-top:var(--space-lg);}
.pr-nav-btn{display:inline-flex;align-items:center;gap:8px;padding:12px 24px;
    border-radius:var(--border-radius-full);font-weight:700;font-size:.875rem;
    text-decoration:none;background:var(--color-white);color:var(--color-text);
    border:1.5px solid var(--border-color);transition:all .2s;cursor:pointer;}
.pr-nav-btn:hover{border-color:var(--pr-orange);color:var(--pr-orange);
...
@media(max-width:767px){
    ...
    .pr-nav{flex-direction:column;}
    .pr-nav-btn{width:100%;justify-content:center;}
}
            ...
            <asp:Repeater ID="rptBadges" runat="server">
                <ItemTemplate>
                    <div class='pr-badge-card <%# (bool)Eval("IsEarned") ? "earned" : "locked" %>'>
                        ...
                        <div class="pr-badge-icon"><%# Eval("Icon") %></div>
...
<%-- ── NAVIGATION BUTTONS ── --%>
<div class="pr-nav">
    <a href="<%: ResolveUrl("~/Student/MyRanking.aspx") %>" class="pr-nav-btn">
        <i class="bi bi-bar-chart-line"></i> <asp:Literal ID="litNavRanking" runat="server" Text="View Ranking" />
    </a>
    <a href="<%: ResolveUrl("~/Student/MyLearning.aspx") %>" class="pr-nav-btn">
        <i class="bi bi-book"></i> <asp:Literal ID="litNavLearning" runat="server" Text="Continue Learning" />
    </a>
    <a href="<%: ResolveUrl("~/Student/PracticeLibrary.aspx") %>" class="pr-nav-btn">
        <i class="bi bi-patch-question"></i> <asp:Literal ID="litNavPractice" runat="server" Text="Practice Library" />
    </a>
    <a href="<%: ResolveUrl("~/Student/VirtualLabs.aspx") %>" class="pr-nav-btn">
        <i class="bi bi-eyedropper"></i> <asp:Literal ID="litNavLabs" runat="server" Text="Virtual Labs" />
    </a>
</div>
