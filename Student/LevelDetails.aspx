<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LevelDetails.aspx.cs"
...
.ld-hero{background:linear-gradient(135deg,#1D4ED8 0%,#2563EB 40%,#4DA8FF 100%);
    ...
    box-shadow:0 10px 36px rgba(37,99,235,.25);}
...
.ld-hero-back{display:inline-flex;align-items:center;gap:6px;font-size:.875rem;
    font-weight:600;color:rgba(255,255,255,.8);text-decoration:none;margin-bottom:var(--space-md);
...
.ld-hero-bar-fill{height:100%;background:linear-gradient(90deg,#FFD84D,#FF9A5C);
    border-radius:var(--border-radius-full);transition:width .8s ease;}
...
.ld-stat{background:var(--color-white);border-radius:var(--border-radius-lg);
...
.ld-stat-icon{width:44px;height:44px;border-radius:var(--border-radius);
...
.ld-section-hd{display:flex;align-items:center;gap:var(--space-sm);margin-bottom:var(--space-lg);}
.ld-section-title{font-family:var(--font-primary);font-size:1.25rem;font-weight:800;color:var(--color-text);}

.ld-roadmap{display:flex;flex-direction:column;gap:var(--space-md);margin-bottom:var(--space-xl);
    position:relative;padding-left:28px;}
.ld-roadmap::before{content:'';position:absolute;left:12px;top:16px;bottom:16px;
    width:3px;background:var(--border-color);border-radius:3px;}
.ld-unit{background:var(--color-white);border-radius:var(--border-radius-xl);
    border:1.5px solid var(--border-color);box-shadow:var(--shadow-sm);padding:var(--space-lg);
    display:flex;gap:var(--space-md);align-items:flex-start;position:relative;
    transition:transform .2s,box-shadow .2s;}
.ld-unit:hover{transform:translateY(-2px);box-shadow:var(--shadow-md);}
.ld-unit-dot{position:absolute;left:-22px;top:24px;width:14px;height:14px;
    border-radius:50%;border:3px solid var(--color-primary);background:var(--color-white);z-index:1;}
.ld-unit-dot.done{background:var(--color-success);border-color:var(--color-success);}
.ld-unit-dot.current{background:var(--student);border-color:var(--student);
...
.ld-unit-bar-fill{height:100%;background:linear-gradient(90deg,var(--student),#FFD84D);
...
.ld-quiz{background:linear-gradient(135deg,var(--student) 0%,#FF9A5C 100%);border-radius:var(--border-radius-xl);
    ...
    position:relative;overflow:hidden;margin-bottom:var(--space-xl);}
.ld-quiz::before{content:'';position:absolute;width:180px;height:180px;border-radius:50%;
    background:rgba(255,255,255,.08);bottom:-50px;left:-30px;pointer-events:none;}
.ld-quiz-icon{width:56px;height:56px;border-radius:var(--border-radius);
...
.ld-locked{background:var(--color-white);border-radius:var(--border-radius-xl);
    border:2px solid var(--border-color);padding:var(--space-3xl);text-align:center;}
...
<asp:Content ID="cSidebar" ContentPlaceHolderID="SidebarMenu" runat="server">
    ...
    <div class="sb-nav-section"><div class="sb-nav-section-label">Learn</div>
        ...
        <a href="#" class="sb-sidebar-item"><i class="bi bi-robot item-icon"></i><span class="item-label">AI Study Companion</span></a>
    </div>
    <div class="sb-nav-section"><div class="sb-nav-section-label">Progress</div>
        ...
        <a href="<%: ResolveUrl("~/Student/Forum.aspx") %>" class="sb-sidebar-item"><i class="bi bi-chat-dots item-icon"></i><span class="item-label">Forum</span></a>
    </div>
...
<asp:Panel ID="pnlLocked" runat="server" Visible="false">
    <div class="ld-locked">
        ...
        <a href="<%: ResolveUrl("~/Student/MyLearning.aspx") %>" class="sb-btn sb-btn-primary sb-btn-sm">
            <i class="bi bi-arrow-left"></i> <asp:Literal ID="litLockedBtn" runat="server" Text="Back" />
        </a>
    </div>
</asp:Panel>
...
<%-- Hero --%>
<div class="ld-hero">
    <a href="<%: ResolveUrl("~/Student/MyLearning.aspx") %>" class="ld-hero-back">
        <i class="bi bi-arrow-left"></i> <asp:Literal ID="litBackText" runat="server" Text="My Learning" />
    </a>
    ...
    <div class="ld-hero-progress">
        <div class="ld-hero-bar"><div class="ld-hero-bar-fill" style="width:<%= LevelProgressPct %>%"></div></div>
        <div class="ld-hero-bar-lbl"><span><asp:Literal ID="litProgressLabel" runat="server" /></span><span><%= LevelProgressPct %>%</span></div>
    </div>
</div>
...
<div class="ld-stats">
    <div class="ld-stat">
        <div class="ld-stat-icon" style="background:#DBEAFE;color:#1D4ED8;"><i class="bi bi-collection-fill"></i></div>
        <div><div class="ld-stat-val"><asp:Literal ID="litStatUnits" runat="server" Text="0" /></div>
        <div class="ld-stat-lbl"><asp:Literal ID="litStatUnitsLbl" runat="server" /></div></div>
    </div>
    <div class="ld-stat">
        <div class="ld-stat-icon" style="background:#DCFCE7;color:#15803D;"><i class="bi bi-check2-circle"></i></div>
        <div><div class="ld-stat-val"><asp:Literal ID="litStatUnitsDone" runat="server" Text="0" /></div>
        <div class="ld-stat-lbl"><asp:Literal ID="litStatUnitsDoneLbl" runat="server" /></div></div>
    </div>
    <div class="ld-stat">
        <div class="ld-stat-icon" style="background:var(--student-light);color:var(--student);"><i class="bi bi-journal-check"></i></div>
        <div><div class="ld-stat-val"><asp:Literal ID="litStatLessons" runat="server" Text="0" /></div>
        <div class="ld-stat-lbl"><asp:Literal ID="litStatLessonsLbl" runat="server" /></div></div>
    </div>
    <div class="ld-stat">
        <div class="ld-stat-icon" style="background:#FFFBEB;color:#B45309;"><i class="bi bi-patch-check-fill"></i></div>
        <div><div class="ld-stat-val"><asp:Literal ID="litStatQuizzes" runat="server" Text="0" /></div>
        <div class="ld-stat-lbl"><asp:Literal ID="litStatQuizzesLbl" runat="server" /></div></div>
    </div>
</div>

<%-- Unit Roadmap --%>
<asp:Panel ID="pnlUnits" runat="server">
    <div class="ld-section-hd">
        <i class="bi bi-signpost-split-fill" style="color:var(--student);font-size:1.25rem;"></i>
        <div class="ld-section-title"><asp:Literal ID="litRoadmapTitle" runat="server" /></div>
    </div>
    <div class="ld-roadmap">
        <asp:Repeater ID="rptUnits" runat="server">
            <ItemTemplate>
                <div class="ld-unit">
                    <div class="ld-unit-dot <%# Eval("DotClass") %>"></div>
                    <div class="ld-unit-body">
                        <div class="ld-unit-name"><%# Eval("Name") %></div>
                        <div class="ld-unit-desc"><%# Eval("Description") %></div>
                        <div class="ld-unit-meta">
                            <span><i class="bi bi-layers"></i> <%# Eval("Subtopics") %> <%# Eval("SubLabel") %></span>
                            <span><i class="bi bi-journal-text"></i> <%# Eval("Lessons") %> <%# Eval("LessonLabel") %></span>
                            <span><i class="bi bi-check2"></i> <%# Eval("Completed") %>/<%# Eval("Lessons") %></span>
                        </div>
                        <div class="ld-unit-bar"><div class="ld-unit-bar-fill" style="width:<%# Eval("Pct") %>%"></div></div>
                        <a href="<%# Eval("Url") %>" class="sb-btn sb-btn-orange sb-btn-xs">
...
<asp:Panel ID="pnlUnitsEmpty" runat="server" Visible="false">
    <div class="sb-empty-state" style="padding:var(--space-2xl) 0;">
        ...
        <div class="empty-title"><asp:Literal ID="litUnitsEmptyTitle" runat="server" /></div>
        <div class="empty-desc"><asp:Literal ID="litUnitsEmptyDesc" runat="server" /></div>
    </div>
</asp:Panel>
...
<asp:Panel ID="pnlQuiz" runat="server" Visible="false">
    <div class="ld-section-hd">
        <i class="bi bi-trophy-fill" style="color:#B45309;font-size:1.25rem;"></i>
        <div class="ld-section-title"><asp:Literal ID="litQuizSection" runat="server" /></div>
    </div>
    <div class="ld-quiz">
        ...
        <a href="#" class="sb-btn sb-btn-white sb-btn-sm">
            <i class="bi bi-play-fill"></i> <asp:Literal ID="litQuizBtn" runat="server" />
        </a>
    </div>
</asp:Panel>
...
<asp:Panel ID="pnlQuizNone" runat="server" Visible="false">
    <div class="sb-alert sb-alert-info"><i class="bi bi-info-circle-fill alert-icon"></i>
        <div class="alert-content"><asp:Literal ID="litQuizNone" runat="server" /></div>
    </div>
</asp:Panel>
