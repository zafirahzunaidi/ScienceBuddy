    ...
    public partial class Lesson1 : Page
    {
        ...
        protected void btnComplete_Click(object sender, EventArgs e)
        {
            ...
            using (var conn = new SqlConnection(ConnStr))
            {
                ...
                if (!already && Tbl("LessonProgress"))
                {
                    ...
                    { int last = Convert.ToInt32(seqCmd.ExecuteScalar()); progId = "LP" + (last + 1).ToString("D3"); }

                    using (var cmd = new SqlCommand(@"INSERT INTO LessonProgress(progressId,studentId,lessonId,isCompleted,completedDate) VALUES(@pid,@s,@l,1,@d)", conn))
                    { cmd.Parameters.AddWithValue("@pid", progId); cmd.Parameters.AddWithValue("@s", studentId);
                      cmd.Parameters.AddWithValue("@l", lessonId); cmd.Parameters.AddWithValue("@d", DateTime.Now); cmd.ExecuteNonQuery(); }
        ...
        private void AwardXP(SqlConnection conn, string studentId, string lessonId)
        {
            try
            {
                ...
                { object r = cmd.ExecuteScalar(); if (r != null && r != DBNull.Value) xpActionId = r.ToString(); }
                ...
                { int last = Convert.ToInt32(cmd.ExecuteScalar()); xtId = "XT" + (last + 1).ToString("D3"); }
                ...
                { cmd.Parameters.AddWithValue("@id", xtId); cmd.Parameters.AddWithValue("@s", studentId);
                  cmd.Parameters.AddWithValue("@a", xpActionId); cmd.Parameters.AddWithValue("@xp", xpAmount);
                  cmd.Parameters.AddWithValue("@dt", DateTime.Today); cmd.ExecuteNonQuery(); }
